...
<h3>
	...
	<?php //Generated by : $Id: print-2.php,v 1.11 2010/01/18 15:20:09 sebastien Exp $
if(!APPLICATION_ENFORCES_ACCESS_CONTROL || (isset($cms_user) && is_a($cms_user, 'CMS_profile_user') && $cms_user->hasModuleClearance('pnews', CLEARANCE_MODULE_VIEW))){
	...
	//SEARCH lastNews TAG START 7_4fc136
	...
	//public search ?
	$public_7_4fc136 = isset($public_search) ? $public_search : false;
	//get search params
	$search_lastNews = new CMS_object_search($objectDefinitions[$objectDefinition_lastNews], $public_7_4fc136);
	...
	//RESULT lastNews TAG START 8_9815a6
	...
	if ($results_lastNews) {
		$object_8_9815a6 = (isset($object[$objectDefinition_lastNews])) ? $object[$objectDefinition_lastNews] : ""; //save previous object search if any
		$replace_8_9815a6 = $replace; //save previous replace vars if any
		$count_8_9815a6 = 0;
		$content_8_9815a6 = $content; //save previous content var if any
		$maxPages_8_9815a6 = $search_lastNews->getMaxPages();
		$maxResults_8_9815a6 = $search_lastNews->getNumRows();
		foreach ($results_lastNews as $object[$objectDefinition_lastNews]) {
			...
			$replace["atm-search"] = array (
				...
				"{firstresult}" => (!$count_8_9815a6) ? 1 : 0,
				"{lastresult}" 	=> ($count_8_9815a6 == sizeof($results_lastNews)-1) ? 1 : 0,
				"{resultcount}" => ($count_8_9815a6+1),
				"{maxpages}"    => $maxPages_8_9815a6,
				"{currentpage}" => ($search_lastNews->getAttribute('page')+1),
				"{maxresults}"  => $maxResults_8_9815a6,
			);
			$content .="
			<div class=\"lastNews\">
			<div class=\"newsTop\">
			...
			}//IF TAG END 10_afcbeb
			$content .="
			<div class=\"spacer\"></div>
			<div class=\"newsBottom\">
			<a class=\"newsAll\" href=\"".CMS_tree::getPageValue("5","url")."\">Toute l'actualite</a>
			";
			//FUNCTION TAG START 11_d38fd7
			$parameters_11_d38fd7 = array ('selected' => CMS_polymod_definition_parsing::replaceVars("3", $replace),);
			$object_11_d38fd7 = &$object[1];
			if (method_exists($object_11_d38fd7, "rss")) {
				$content .= CMS_polymod_definition_parsing::replaceVars($object_11_d38fd7->rss($parameters_11_d38fd7, array (
					0 =>
					array (
						'textnode' => '
						',
				...
				CMS_grandFather::raiseError("Malformed atm-function tag : can't found method rss on object : ".get_class($object_11_d38fd7));
			}
			//FUNCTION TAG END 11_d38fd7
			$content .="
			</div>
			</div>
			</div>
			...
			$content_8_9815a6.= CMS_polymod_definition_parsing::replaceVars($content, $replace);
		}
		$content = $content_8_9815a6; //retrieve previous content var if any
		$replace = $replace_8_9815a6; //retrieve previous replace vars if any
		$object[$objectDefinition_lastNews] = $object_8_9815a6; //retrieve previous object search if any
	}
	//RESULT lastNews TAG END 8_9815a6
	//destroy search and results lastNews objects
	unset($search_lastNews);
	unset($results_lastNews);
	//SEARCH lastNews TAG END 7_4fc136
