<?php //Generated on Mon, 01 Feb 2010 16:33:58 +0100 by Automne (TM) 4.0.0
...
<h3>
	...
	<?php //Generated by : $Id: print-2.php,v 1.13 2010/02/01 16:08:06 sebastien Exp $
if(!APPLICATION_ENFORCES_ACCESS_CONTROL || (isset($cms_user) && is_a($cms_user, 'CMS_profile_user') && $cms_user->hasModuleClearance('pnews', CLEARANCE_MODULE_VIEW))){
	...
	//SEARCH lastNews TAG START 7_e148ef
	...
	//public search ?
	$public_7_e148ef = isset($public_search) ? $public_search : false;
	//get search params
	$search_lastNews = new CMS_object_search($objectDefinitions[$objectDefinition_lastNews], $public_7_e148ef);
	...
	//RESULT lastNews TAG START 8_d8e82d
	...
	if ($results_lastNews) {
		$object_8_d8e82d = (isset($object[$objectDefinition_lastNews])) ? $object[$objectDefinition_lastNews] : ""; //save previous object search if any
		$replace_8_d8e82d = $replace; //save previous replace vars if any
		$count_8_d8e82d = 0;
		$content_8_d8e82d = $content; //save previous content var if any
		$maxPages_8_d8e82d = $search_lastNews->getMaxPages();
		$maxResults_8_d8e82d = $search_lastNews->getNumRows();
		foreach ($results_lastNews as $object[$objectDefinition_lastNews]) {
			...
			$replace["atm-search"] = array (
				...
				"{firstresult}" => (!$count_8_d8e82d) ? 1 : 0,
				"{lastresult}" 	=> ($count_8_d8e82d == sizeof($results_lastNews)-1) ? 1 : 0,
				"{resultcount}" => ($count_8_d8e82d+1),
				"{maxpages}"    => $maxPages_8_d8e82d,
				"{currentpage}" => ($search_lastNews->getAttribute('page')+1),
				"{maxresults}"  => $maxResults_8_d8e82d,
			);
			$content .="
			<div class=\"lastNews\">
			<div class=\"newsTop\">
			...
			}//IF TAG END 10_7011bf
			$content .="
			<div class=\"spacer\"></div>
			<div class=\"newsBottom\">
			<a class=\"newsAll\" href=\"".CMS_tree::getPageValue("5","url")."\">Toute l'actualite</a>
			";
			//FUNCTION TAG START 11_f90c2d
			$parameters_11_f90c2d = array ('selected' => CMS_polymod_definition_parsing::replaceVars("3", $replace),);
			$object_11_f90c2d = &$object[1];
			if (method_exists($object_11_f90c2d, "rss")) {
				$content .= CMS_polymod_definition_parsing::replaceVars($object_11_f90c2d->rss($parameters_11_f90c2d, array (
					0 =>
					array (
						'textnode' => '
						',
				...
				CMS_grandFather::raiseError("Malformed atm-function tag : can't found method rss on object : ".get_class($object_11_f90c2d));
			}
			//FUNCTION TAG END 11_f90c2d
			$content .="
			</div>
			</div>
			</div>
			...
			$content_8_d8e82d.= CMS_polymod_definition_parsing::replaceVars($content, $replace);
		}
		$content = $content_8_d8e82d; //retrieve previous content var if any
		$replace = $replace_8_d8e82d; //retrieve previous replace vars if any
		$object[$objectDefinition_lastNews] = $object_8_d8e82d; //retrieve previous object search if any
	}
	//RESULT lastNews TAG END 8_d8e82d
	//destroy search and results lastNews objects
	unset($search_lastNews);
	unset($results_lastNews);
	//SEARCH lastNews TAG END 7_e148ef
