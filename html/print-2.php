...
<h3>
	...
	<?php //Generated by : $Id: print-2.php,v 1.7 2009/06/23 16:00:41 sebastien Exp $
$content = "";
$replace = "";
...
//SEARCH lastNews TAG START 7_b1681b
...
//public search ?
$public_7_b1681b = isset($public_search) ? $public_search : false;
//get search params
$search_lastNews = new CMS_object_search($objectDefinitions[$objectDefinition_lastNews], $public_7_b1681b);
...
//RESULT lastNews TAG START 8_524fd1
...
if ($results_lastNews) {
	$object_8_524fd1 = $object[$objectDefinition_lastNews]; //save previous object search if any
	$replace_8_524fd1 = $replace; //save previous replace vars if any
	$count_8_524fd1 = 0;
	$content_8_524fd1 = $content; //save previous content var if any
	$maxPages_8_524fd1 = $search_lastNews->getMaxPages();
	$maxResults_8_524fd1 = $search_lastNews->getNumRows();
	foreach ($results_lastNews as $object[$objectDefinition_lastNews]) {
		...
		$replace["atm-search"] = array (
			...
			"{firstresult}" => (!$count_8_524fd1) ? 1 : 0,
			"{lastresult}" 	=> ($count_8_524fd1 == sizeof($results_lastNews)-1) ? 1 : 0,
			"{resultcount}" => ($count_8_524fd1+1),
			"{maxpages}"    => $maxPages_8_524fd1,
			"{currentpage}" => ($search_lastNews->getAttribute('page')+1),
			"{maxresults}"  => $maxResults_8_524fd1,
		);
		$content .="
		<div class=\"lastNews\">
		<div class=\"newsTop\">
		...
		}//IF TAG END 10_54e61f
		$content .="
		<div class=\"spacer\"></div>
		<div class=\"newsBottom\">
		<a class=\"newsAll\" href=\"".CMS_tree::getPageValue("5","url")."\">Toute l'actualite</a>
		";
		//FUNCTION TAG START 11_c8136e
		$parameters_11_c8136e = array ('selected' => CMS_polymod_definition_parsing::replaceVars("3", $replace),);
		$object_11_c8136e = &$object[1];
		if (method_exists($object_11_c8136e, "rss")) {
			$content .= CMS_polymod_definition_parsing::replaceVars($object_11_c8136e->rss($parameters_11_c8136e, array (
				0 =>
				array (
					'textnode' => '
					',
			...
			CMS_grandFather::raiseError("Malformed atm-function tag : can't found method rss on object : ".get_class($object_11_c8136e));
		}
		//FUNCTION TAG END 11_c8136e
		$content .="
		</div>
		</div>
		</div>
		...
		$content_8_524fd1.= CMS_polymod_definition_parsing::replaceVars($content, $replace);
	}
	$content = $content_8_524fd1; //retrieve previous content var if any
	$replace = $replace_8_524fd1; //retrieve previous replace vars if any
	$object[$objectDefinition_lastNews] = $object_8_524fd1; //retrieve previous object search if any
}
//RESULT lastNews TAG END 8_524fd1
//destroy search and results lastNews objects
unset($search_lastNews);
unset($results_lastNews);
//SEARCH lastNews TAG END 7_b1681b
