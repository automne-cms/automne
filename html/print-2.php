<?php //Generated on Tue, 09 Mar 2010 11:58:46 +0100 by Automne (TM) 4.0.1
require_once($_SERVER["DOCUMENT_ROOT"]."/cms_rc_frontend.php");
if (!isset($cms_page_included) && !$_POST && !$_GET) {
	CMS_view::redirect('http://automne4.trunk/web/demo/print-2-accueil.php', true, 301);
}
 ?>
...
if(!APPLICATION_ENFORCES_ACCESS_CONTROL || (isset($cms_user) && is_a($cms_user, 'CMS_profile_user') && $cms_user->hasModuleClearance('pnews', CLEARANCE_MODULE_VIEW))){
	...
	//SEARCH lastNews TAG START 7_62e497
	...
	//public search ?
	$public_7_62e497 = isset($public_search) ? $public_search : false;
	//get search params
	$search_lastNews = new CMS_object_search($objectDefinitions[$objectDefinition_lastNews], $public_7_62e497);
	...
	//RESULT lastNews TAG START 8_d8e876
	...
	if ($results_lastNews) {
		$object_8_d8e876 = (isset($object[$objectDefinition_lastNews])) ? $object[$objectDefinition_lastNews] : ""; //save previous object search if any
		$replace_8_d8e876 = $replace; //save previous replace vars if any
		$count_8_d8e876 = 0;
		$content_8_d8e876 = $content; //save previous content var if any
		$maxPages_8_d8e876 = $search_lastNews->getMaxPages();
		$maxResults_8_d8e876 = $search_lastNews->getNumRows();
		foreach ($results_lastNews as $object[$objectDefinition_lastNews]) {
			...
			$replace["atm-search"] = array (
				...
				"{firstresult}" => (!$count_8_d8e876) ? 1 : 0,
				"{lastresult}" 	=> ($count_8_d8e876 == sizeof($results_lastNews)-1) ? 1 : 0,
				"{resultcount}" => ($count_8_d8e876+1),
				"{maxpages}"    => $maxPages_8_d8e876,
				"{currentpage}" => ($search_lastNews->getAttribute('page')+1),
				"{maxresults}"  => $maxResults_8_d8e876,
			);
			$content .="
			<div class=\"lastNews\">
			<div class=\"newsTop\">
			...
			}//IF TAG END 10_a2d8cf
			$content .="
			<div class=\"spacer\"></div>
			<div class=\"newsBottom\">
			<a class=\"newsAll\" href=\"".CMS_tree::getPageValue("5","url")."\">Toute l'actualite</a>
			";
			//FUNCTION TAG START 11_2586de
			$parameters_11_2586de = array ('selected' => CMS_polymod_definition_parsing::replaceVars("3", $replace),);
			$object_11_2586de = &$object[1];
			if (method_exists($object_11_2586de, "rss")) {
				$content .= CMS_polymod_definition_parsing::replaceVars($object_11_2586de->rss($parameters_11_2586de, array (
					0 =>
					array (
						'textnode' => '
						',
				...
				CMS_grandFather::raiseError("Malformed atm-function tag : can't found method rss on object : ".get_class($object_11_2586de));
			}
			//FUNCTION TAG END 11_2586de
			$content .="
			</div>
			</div>
			</div>
			...
			$content_8_d8e876.= CMS_polymod_definition_parsing::replaceVars($content, $replace);
		}
		$content = $content_8_d8e876; //retrieve previous content var if any
		$replace = $replace_8_d8e876; //retrieve previous replace vars if any
		$object[$objectDefinition_lastNews] = $object_8_d8e876; //retrieve previous object search if any
	}
	//RESULT lastNews TAG END 8_d8e876
	//destroy search and results lastNews objects
	unset($search_lastNews);
	unset($results_lastNews);
	//SEARCH lastNews TAG END 7_62e497
...
<div align="center">
	<small>
				...
				Page  "Accueil" (http://automne4.trunk/web/demo/2-accueil.php)
				<br />
