...
<div class="text"><p>Il est possible d'ajouter au noyau logiciel un ensemble de modules pour ajouter des fonctionnalit&eacute;s propres aux besoins de chaque site.</p><p>Par d&eacute;faut Automne 4 contient les modules les plus courants : <strong>M&eacute;diath&egrave;que, Gestion des Actualit&eacute;s, Cr&eacute;ation de formulaires, Cr&eacute;ation d'Alias de pages. </strong></p><h3>Il vous est cependant possible d'ajouter autant de modules suppl&eacute;mentaires que vous le souhaitez !</h3> <h2>G&eacute;n&eacute;rateur de modules&nbsp; POLYMOD</h2></div>
	...
	<div class="imgRight">
		<?php //Generated by : $Id: print-27.php,v 1.4 2009/10/28 16:31:35 sebastien Exp $
$content = "";
$replace = "";
...
//SEARCH mediaresult TAG START 16_c2b084
$objectDefinition_mediaresult = '2';
if (!isset($objectDefinitions[$objectDefinition_mediaresult])) {
	$objectDefinitions[$objectDefinition_mediaresult] = new CMS_poly_object_definition($objectDefinition_mediaresult);
}
//public search ?
$public_16_c2b084 = isset($public_search) ? $public_search : false;
//get search params
$search_mediaresult = new CMS_object_search($objectDefinitions[$objectDefinition_mediaresult], $public_16_c2b084);
$launchSearch_mediaresult = true;
//add search conditions if any
if (isset($blockAttributes['search']['mediaresult']['item'])) {
	$values_17_94d491 = array (
		'search' => 'mediaresult',
		'type' => 'item',
		'value' => 'block',
		'mandatory' => 'true',
	);
	$values_17_94d491['value'] = $blockAttributes['search']['mediaresult']['item'];
	if ($values_17_94d491['type'] == 'publication date after' || $values_17_94d491['type'] == 'publication date before') {
		//convert DB format to current language format
		$dt = new CMS_date();
		$dt->setFromDBValue($values_17_94d491['value']);
		$values_17_94d491['value'] = $dt->getLocalizedDate($cms_language->getDateFormat());
	}
	$launchSearch_mediaresult = (CMS_polymod_definition_parsing::addSearchCondition($search_mediaresult, $values_17_94d491)) ? $launchSearch_mediaresult : false;
} elseif (true == true) {
	//search parameter is mandatory and no value found
	$launchSearch_mediaresult = false;
}
//RESULT mediaresult TAG START 18_dbfa53
...
if ($results_mediaresult) {
	$object_18_dbfa53 = (isset($object[$objectDefinition_mediaresult])) ? $object[$objectDefinition_mediaresult] : ""; //save previous object search if any
	$replace_18_dbfa53 = $replace; //save previous replace vars if any
	$count_18_dbfa53 = 0;
	$content_18_dbfa53 = $content; //save previous content var if any
	$maxPages_18_dbfa53 = $search_mediaresult->getMaxPages();
	$maxResults_18_dbfa53 = $search_mediaresult->getNumRows();
	foreach ($results_mediaresult as $object[$objectDefinition_mediaresult]) {
		$content = "";
		$replace["atm-search"] = array (
			"{resultid}" 	=> (isset($resultID_mediaresult)) ? $resultID_mediaresult : $object[$objectDefinition_mediaresult]->getID(),
			"{firstresult}" => (!$count_18_dbfa53) ? 1 : 0,
			"{lastresult}" 	=> ($count_18_dbfa53 == sizeof($results_mediaresult)-1) ? 1 : 0,
			"{resultcount}" => ($count_18_dbfa53+1),
			"{maxpages}"    => $maxPages_18_dbfa53,
			"{currentpage}" => ($search_mediaresult->getAttribute('page')+1),
			"{maxresults}"  => $maxResults_18_dbfa53,
		);
		//IF TAG START 19_4d3be1
		...
		if ($ifcondition) {
			...
			if ($func()) {
				...
				}//IF TAG END 20_035ba3
				$content .=" ".$object[2]->getValue('label','')."</a>
				";
				//IF TAG START 21_c44999
				...
				}//IF TAG END 21_c44999
			}
		}//IF TAG END 19_4d3be1
		//IF TAG START 22_e7bae3
		$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('fileExtension',''))." == 'flv'", $replace);
		if ($ifcondition) {
			$func = create_function("","return (".$ifcondition.");");
			if ($func()) {
				//IF TAG START 23_3eeb6e
				$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
				if ($ifcondition) {
					$func = create_function("","return (".$ifcondition.");");
					if ($func()) {
				...
				}//IF TAG END 23_3eeb6e
				//IF TAG START 24_52485b
				$ifcondition = CMS_polymod_definition_parsing::replaceVars("!".CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
				if ($ifcondition) {
					$func = create_function("","return (".$ifcondition.");");
					if ($func()) {
		...
		}//IF TAG END 22_e7bae3
		//IF TAG START 25_7deb19
		...
		if ($ifcondition) {
			...
			if ($func()) {
				...
				}//IF TAG END 26_375737
			}
		}//IF TAG END 25_7deb19
		//IF TAG START 27_ce3266
		...
		if ($ifcondition) {
			...
			if ($func()) {
				...
				}//IF TAG END 28_9179f2
				//IF TAG START 29_695d6a
				$ifcondition = CMS_polymod_definition_parsing::replaceVars("!".CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
				if ($ifcondition) {
					$func = create_function("","return (".$ifcondition.");");
					if ($func()) {
		...
		}//IF TAG END 27_ce3266
		$count_18_dbfa53++;
		//do all result vars replacement
		$content_18_dbfa53.= CMS_polymod_definition_parsing::replaceVars($content, $replace);
	}
	$content = $content_18_dbfa53; //retrieve previous content var if any
	$replace = $replace_18_dbfa53; //retrieve previous replace vars if any
	$object[$objectDefinition_mediaresult] = $object_18_dbfa53; //retrieve previous object search if any
}
//RESULT mediaresult TAG END 18_dbfa53
//destroy search and results mediaresult objects
unset($search_mediaresult);
unset($results_mediaresult);
//SEARCH mediaresult TAG END 16_c2b084
