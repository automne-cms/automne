<?php //Generated on Mon, 18 Jan 2010 17:55:49 +0100 by Automne (TM) 4.0.0
...
<div class="text"><p>Il est possible d'ajouter au noyau logiciel un ensemble de modules pour ajouter des fonctionnalit&eacute;s propres aux besoins de chaque site.</p><p>Par d&eacute;faut Automne 4 contient les modules les plus courants : <strong>M&eacute;diath&egrave;que, Gestion des Actualit&eacute;s, Cr&eacute;ation de formulaires, Cr&eacute;ation d'Alias de pages. </strong></p><h3>Il vous est cependant possible d'ajouter autant de modules suppl&eacute;mentaires que vous le souhaitez !</h3> <h2>G&eacute;n&eacute;rateur de modules&nbsp; POLYMOD</h2></div>
	...
	<div class="imgRight">
		<?php //Generated by : $Id: print-27.php,v 1.7 2010/01/18 16:49:34 sebastien Exp $
if(!APPLICATION_ENFORCES_ACCESS_CONTROL || (isset($cms_user) && is_a($cms_user, 'CMS_profile_user') && $cms_user->hasModuleClearance('pmedia', CLEARANCE_MODULE_VIEW))){
	...
	//SEARCH mediaresult TAG START 16_2edcf8
	$objectDefinition_mediaresult = '2';
	if (!isset($objectDefinitions[$objectDefinition_mediaresult])) {
		$objectDefinitions[$objectDefinition_mediaresult] = new CMS_poly_object_definition($objectDefinition_mediaresult);
	}
	//public search ?
	$public_16_2edcf8 = isset($public_search) ? $public_search : false;
	//get search params
	$search_mediaresult = new CMS_object_search($objectDefinitions[$objectDefinition_mediaresult], $public_16_2edcf8);
	$launchSearch_mediaresult = true;
	//add search conditions if any
	if (isset($blockAttributes['search']['mediaresult']['item'])) {
		$values_17_e74394 = array (
			'search' => 'mediaresult',
			'type' => 'item',
			'value' => 'block',
			'mandatory' => 'true',
		);
		$values_17_e74394['value'] = $blockAttributes['search']['mediaresult']['item'];
		if ($values_17_e74394['type'] == 'publication date after' || $values_17_e74394['type'] == 'publication date before') {
			//convert DB format to current language format
			$dt = new CMS_date();
			$dt->setFromDBValue($values_17_e74394['value']);
			$values_17_e74394['value'] = $dt->getLocalizedDate($cms_language->getDateFormat());
		}
		$launchSearch_mediaresult = (CMS_polymod_definition_parsing::addSearchCondition($search_mediaresult, $values_17_e74394)) ? $launchSearch_mediaresult : false;
	} elseif (true == true) {
		//search parameter is mandatory and no value found
		$launchSearch_mediaresult = false;
	}
	//RESULT mediaresult TAG START 18_148ee4
	...
	if ($results_mediaresult) {
		$object_18_148ee4 = (isset($object[$objectDefinition_mediaresult])) ? $object[$objectDefinition_mediaresult] : ""; //save previous object search if any
		$replace_18_148ee4 = $replace; //save previous replace vars if any
		$count_18_148ee4 = 0;
		$content_18_148ee4 = $content; //save previous content var if any
		$maxPages_18_148ee4 = $search_mediaresult->getMaxPages();
		$maxResults_18_148ee4 = $search_mediaresult->getNumRows();
		foreach ($results_mediaresult as $object[$objectDefinition_mediaresult]) {
			$content = "";
			$replace["atm-search"] = array (
				"{resultid}" 	=> (isset($resultID_mediaresult)) ? $resultID_mediaresult : $object[$objectDefinition_mediaresult]->getID(),
				"{firstresult}" => (!$count_18_148ee4) ? 1 : 0,
				"{lastresult}" 	=> ($count_18_148ee4 == sizeof($results_mediaresult)-1) ? 1 : 0,
				"{resultcount}" => ($count_18_148ee4+1),
				"{maxpages}"    => $maxPages_18_148ee4,
				"{currentpage}" => ($search_mediaresult->getAttribute('page')+1),
				"{maxresults}"  => $maxResults_18_148ee4,
			);
			//IF TAG START 19_60e30a
			...
			if ($ifcondition) {
				...
				if ($func()) {
					...
					}//IF TAG END 20_f1c67a
					$content .=" ".$object[2]->getValue('label','')."</a>
					";
					//IF TAG START 21_9eaaaf
					...
					}//IF TAG END 21_9eaaaf
				}
			}//IF TAG END 19_60e30a
			//IF TAG START 22_5f8fd4
			$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('fileExtension',''))." == 'flv'", $replace);
			if ($ifcondition) {
				$func = create_function("","return (".$ifcondition.");");
				if ($func()) {
					//IF TAG START 23_3b020b
					$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
					if ($ifcondition) {
						$func = create_function("","return (".$ifcondition.");");
						if ($func()) {
					...
					}//IF TAG END 23_3b020b
					//IF TAG START 24_c69755
					$ifcondition = CMS_polymod_definition_parsing::replaceVars("!".CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
					if ($ifcondition) {
						$func = create_function("","return (".$ifcondition.");");
						if ($func()) {
			...
			}//IF TAG END 22_5f8fd4
			//IF TAG START 25_f0a0eb
			...
			if ($ifcondition) {
				...
				if ($func()) {
					...
					}//IF TAG END 26_01aaa6
				}
			}//IF TAG END 25_f0a0eb
			//IF TAG START 27_dab5e1
			...
			if ($ifcondition) {
				...
				if ($func()) {
					...
					}//IF TAG END 28_233fd5
					//IF TAG START 29_41f4ac
					$ifcondition = CMS_polymod_definition_parsing::replaceVars("!".CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
					if ($ifcondition) {
						$func = create_function("","return (".$ifcondition.");");
						if ($func()) {
					...
					}//IF TAG END 29_41f4ac
					$content .="
					</div>
					";
				}
			}//IF TAG END 27_dab5e1
			$count_18_148ee4++;
			//do all result vars replacement
			$content_18_148ee4.= CMS_polymod_definition_parsing::replaceVars($content, $replace);
		}
		$content = $content_18_148ee4; //retrieve previous content var if any
		$replace = $replace_18_148ee4; //retrieve previous replace vars if any
		$object[$objectDefinition_mediaresult] = $object_18_148ee4; //retrieve previous object search if any
	}
	//RESULT mediaresult TAG END 18_148ee4
	//destroy search and results mediaresult objects
	unset($search_mediaresult);
	unset($results_mediaresult);
	//SEARCH mediaresult TAG END 16_2edcf8
