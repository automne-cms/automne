<?php //Generated on Tue, 23 Jun 2009 18:05:09 +0200 by Automne (TM) 4.0.0rc1
...
<div class="text"><p>Il est possible d'ajouter au noyau logiciel un ensemble de modules pour ajouter des fonctionnalit&eacute;s propres aux besoins de chaque site.</p><p>Par d&eacute;faut Automne 4 contient les modules les plus courants : <strong>M&eacute;diath&egrave;que, Gestion des Actualit&eacute;s, Cr&eacute;ation de formulaires, Cr&eacute;ation d'Alias de pages. </strong></p><h3>Il vous est cependant possible d'ajouter autant de modules suppl&eacute;mentaires que vous le souhaitez !</h3> <h2>G&eacute;n&eacute;rateur de modules&nbsp; POLYMOD</h2></div>
	...
	<div class="imgRight">
		<?php //Generated by : $Id: print-27.php,v 1.2 2009/06/23 16:00:42 sebastien Exp $
$content = "";
$replace = "";
...
//SEARCH mediaresult TAG START 16_5f0141
$objectDefinition_mediaresult = '2';
if (!isset($objectDefinitions[$objectDefinition_mediaresult])) {
	$objectDefinitions[$objectDefinition_mediaresult] = new CMS_poly_object_definition($objectDefinition_mediaresult);
}
//public search ?
$public_16_5f0141 = isset($public_search) ? $public_search : false;
//get search params
$search_mediaresult = new CMS_object_search($objectDefinitions[$objectDefinition_mediaresult], $public_16_5f0141);
$launchSearch_mediaresult = true;
//add search conditions if any
if (isset($blockAttributes['search']['mediaresult']['item'])) {
	$values_17_8c5635 = array (
		'search' => 'mediaresult',
		'type' => 'item',
		'value' => 'block',
		'mandatory' => 'true',
	);
	$values_17_8c5635['value'] = $blockAttributes['search']['mediaresult']['item'];
	if ($values_17_8c5635['type'] == 'publication date after' || $values_17_8c5635['type'] == 'publication date before') {
		//convert DB format to current language format
		$dt = new CMS_date();
		$dt->setFromDBValue($values_17_8c5635['value']);
		$values_17_8c5635['value'] = $dt->getLocalizedDate($cms_language->getDateFormat());
	}
	$launchSearch_mediaresult = (CMS_polymod_definition_parsing::addSearchCondition($search_mediaresult, $values_17_8c5635)) ? $launchSearch_mediaresult : false;
} elseif (true == true) {
	//search parameter is mandatory and no value found
	$launchSearch_mediaresult = false;
}
//RESULT mediaresult TAG START 18_f7468f
...
if ($results_mediaresult) {
	$object_18_f7468f = $object[$objectDefinition_mediaresult]; //save previous object search if any
	$replace_18_f7468f = $replace; //save previous replace vars if any
	$count_18_f7468f = 0;
	$content_18_f7468f = $content; //save previous content var if any
	$maxPages_18_f7468f = $search_mediaresult->getMaxPages();
	$maxResults_18_f7468f = $search_mediaresult->getNumRows();
	foreach ($results_mediaresult as $object[$objectDefinition_mediaresult]) {
		$content = "";
		$replace["atm-search"] = array (
			"{resultid}" 	=> (isset($resultID_mediaresult)) ? $resultID_mediaresult : $object[$objectDefinition_mediaresult]->getID(),
			"{firstresult}" => (!$count_18_f7468f) ? 1 : 0,
			"{lastresult}" 	=> ($count_18_f7468f == sizeof($results_mediaresult)-1) ? 1 : 0,
			"{resultcount}" => ($count_18_f7468f+1),
			"{maxpages}"    => $maxPages_18_f7468f,
			"{currentpage}" => ($search_mediaresult->getAttribute('page')+1),
			"{maxresults}"  => $maxResults_18_f7468f,
		);
		//IF TAG START 19_2e8337
		...
		if ($ifcondition) {
			...
			if ($func()) {
				...
				}//IF TAG END 20_8d114b
				$content .=" ".$object[2]->getValue('label','')."</a>
				";
				//IF TAG START 21_18ecf8
				...
				}//IF TAG END 21_18ecf8
			}
		}//IF TAG END 19_2e8337
		//IF TAG START 22_54266a
		$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('fileExtension',''))." == 'flv'", $replace);
		if ($ifcondition) {
			$func = create_function("","return (".$ifcondition.");");
			if ($func()) {
				//IF TAG START 23_0c8230
				$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
				if ($ifcondition) {
					$func = create_function("","return (".$ifcondition.");");
					if ($func()) {
				...
				}//IF TAG END 23_0c8230
				//IF TAG START 24_91e3a4
				$ifcondition = CMS_polymod_definition_parsing::replaceVars("!".CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
				if ($ifcondition) {
					$func = create_function("","return (".$ifcondition.");");
					if ($func()) {
		...
		}//IF TAG END 22_54266a
		//IF TAG START 25_6dda4e
		...
		if ($ifcondition) {
			...
			if ($func()) {
				...
				}//IF TAG END 26_2c99bd
			}
		}//IF TAG END 25_6dda4e
		//IF TAG START 27_856205
		...
		if ($ifcondition) {
			...
			if ($func()) {
				...
				}//IF TAG END 28_00e234
				//IF TAG START 29_3776c3
				$ifcondition = CMS_polymod_definition_parsing::replaceVars("!".CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
				if ($ifcondition) {
					$func = create_function("","return (".$ifcondition.");");
					if ($func()) {
		...
		}//IF TAG END 27_856205
		$count_18_f7468f++;
		//do all result vars replacement
		$content_18_f7468f.= CMS_polymod_definition_parsing::replaceVars($content, $replace);
	}
	$content = $content_18_f7468f; //retrieve previous content var if any
	$replace = $replace_18_f7468f; //retrieve previous replace vars if any
	$object[$objectDefinition_mediaresult] = $object_18_f7468f; //retrieve previous object search if any
}
//RESULT mediaresult TAG END 18_f7468f
//destroy search and results mediaresult objects
unset($search_mediaresult);
unset($results_mediaresult);
//SEARCH mediaresult TAG END 16_5f0141
