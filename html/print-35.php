<?php //Generated on Mon, 01 Feb 2010 16:34:22 +0100 by Automne (TM) 4.0.0
...
<h2>Principe de gestion des droits</h2>
	...
	<div class="imgRight">
		<?php //Generated by : $Id: print-35.php,v 1.8 2010/02/01 16:08:06 sebastien Exp $
if(!APPLICATION_ENFORCES_ACCESS_CONTROL || (isset($cms_user) && is_a($cms_user, 'CMS_profile_user') && $cms_user->hasModuleClearance('pmedia', CLEARANCE_MODULE_VIEW))){
	...
	//SEARCH mediaresult TAG START 16_301e4f
	$objectDefinition_mediaresult = '2';
	if (!isset($objectDefinitions[$objectDefinition_mediaresult])) {
		$objectDefinitions[$objectDefinition_mediaresult] = new CMS_poly_object_definition($objectDefinition_mediaresult);
	}
	//public search ?
	$public_16_301e4f = isset($public_search) ? $public_search : false;
	//get search params
	$search_mediaresult = new CMS_object_search($objectDefinitions[$objectDefinition_mediaresult], $public_16_301e4f);
	$launchSearch_mediaresult = true;
	//add search conditions if any
	if (isset($blockAttributes['search']['mediaresult']['item'])) {
		$values_17_935174 = array (
			'search' => 'mediaresult',
			'type' => 'item',
			'value' => 'block',
			'mandatory' => 'true',
		);
		$values_17_935174['value'] = $blockAttributes['search']['mediaresult']['item'];
		if ($values_17_935174['type'] == 'publication date after' || $values_17_935174['type'] == 'publication date before') {
			//convert DB format to current language format
			$dt = new CMS_date();
			$dt->setFromDBValue($values_17_935174['value']);
			$values_17_935174['value'] = $dt->getLocalizedDate($cms_language->getDateFormat());
		}
		$launchSearch_mediaresult = (CMS_polymod_definition_parsing::addSearchCondition($search_mediaresult, $values_17_935174)) ? $launchSearch_mediaresult : false;
	} elseif (true == true) {
		//search parameter is mandatory and no value found
		$launchSearch_mediaresult = false;
	}
	//RESULT mediaresult TAG START 18_f01291
	...
	if ($results_mediaresult) {
		$object_18_f01291 = (isset($object[$objectDefinition_mediaresult])) ? $object[$objectDefinition_mediaresult] : ""; //save previous object search if any
		$replace_18_f01291 = $replace; //save previous replace vars if any
		$count_18_f01291 = 0;
		$content_18_f01291 = $content; //save previous content var if any
		$maxPages_18_f01291 = $search_mediaresult->getMaxPages();
		$maxResults_18_f01291 = $search_mediaresult->getNumRows();
		foreach ($results_mediaresult as $object[$objectDefinition_mediaresult]) {
			$content = "";
			$replace["atm-search"] = array (
				"{resultid}" 	=> (isset($resultID_mediaresult)) ? $resultID_mediaresult : $object[$objectDefinition_mediaresult]->getID(),
				"{firstresult}" => (!$count_18_f01291) ? 1 : 0,
				"{lastresult}" 	=> ($count_18_f01291 == sizeof($results_mediaresult)-1) ? 1 : 0,
				"{resultcount}" => ($count_18_f01291+1),
				"{maxpages}"    => $maxPages_18_f01291,
				"{currentpage}" => ($search_mediaresult->getAttribute('page')+1),
				"{maxresults}"  => $maxResults_18_f01291,
			);
			//IF TAG START 19_792e55
			...
			if ($ifcondition) {
				...
				if ($func()) {
					...
					}//IF TAG END 20_689c42
					$content .=" ".$object[2]->getValue('label','')."</a>
					";
					//IF TAG START 21_078a5d
					...
					}//IF TAG END 21_078a5d
				}
			}//IF TAG END 19_792e55
			//IF TAG START 22_26b43e
			$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('fileExtension',''))." == 'flv'", $replace);
			if ($ifcondition) {
				$func = create_function("","return (".$ifcondition.");");
				if ($func()) {
					//IF TAG START 23_9953a0
					$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
					if ($ifcondition) {
						$func = create_function("","return (".$ifcondition.");");
						if ($func()) {
					...
					}//IF TAG END 23_9953a0
					//IF TAG START 24_1a5a52
					$ifcondition = CMS_polymod_definition_parsing::replaceVars("!".CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
					if ($ifcondition) {
						$func = create_function("","return (".$ifcondition.");");
						if ($func()) {
			...
			}//IF TAG END 22_26b43e
			//IF TAG START 25_f3796f
			...
			if ($ifcondition) {
				...
				if ($func()) {
					...
					}//IF TAG END 26_12d21b
				}
			}//IF TAG END 25_f3796f
			//IF TAG START 27_5acc32
			...
			if ($ifcondition) {
				...
				if ($func()) {
					...
					}//IF TAG END 28_63284e
					//IF TAG START 29_8856bd
					$ifcondition = CMS_polymod_definition_parsing::replaceVars("!".CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
					if ($ifcondition) {
						$func = create_function("","return (".$ifcondition.");");
						if ($func()) {
					...
					}//IF TAG END 29_8856bd
					$content .="
					</div>
					";
				}
			}//IF TAG END 27_5acc32
			$count_18_f01291++;
			//do all result vars replacement
			$content_18_f01291.= CMS_polymod_definition_parsing::replaceVars($content, $replace);
		}
		$content = $content_18_f01291; //retrieve previous content var if any
		$replace = $replace_18_f01291; //retrieve previous replace vars if any
		$object[$objectDefinition_mediaresult] = $object_18_f01291; //retrieve previous object search if any
	}
	//RESULT mediaresult TAG END 18_f01291
	//destroy search and results mediaresult objects
	unset($search_mediaresult);
	unset($results_mediaresult);
	//SEARCH mediaresult TAG END 16_301e4f
