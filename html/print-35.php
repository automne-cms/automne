<?php //Generated on Mon, 18 Jan 2010 17:55:38 +0100 by Automne (TM) 4.0.0
...
<h2>Principe de gestion des droits</h2>
	...
	<div class="imgRight">
		<?php //Generated by : $Id: print-35.php,v 1.7 2010/01/18 16:49:34 sebastien Exp $
if(!APPLICATION_ENFORCES_ACCESS_CONTROL || (isset($cms_user) && is_a($cms_user, 'CMS_profile_user') && $cms_user->hasModuleClearance('pmedia', CLEARANCE_MODULE_VIEW))){
	...
	//SEARCH mediaresult TAG START 16_2aad13
	$objectDefinition_mediaresult = '2';
	if (!isset($objectDefinitions[$objectDefinition_mediaresult])) {
		$objectDefinitions[$objectDefinition_mediaresult] = new CMS_poly_object_definition($objectDefinition_mediaresult);
	}
	//public search ?
	$public_16_2aad13 = isset($public_search) ? $public_search : false;
	//get search params
	$search_mediaresult = new CMS_object_search($objectDefinitions[$objectDefinition_mediaresult], $public_16_2aad13);
	$launchSearch_mediaresult = true;
	//add search conditions if any
	if (isset($blockAttributes['search']['mediaresult']['item'])) {
		$values_17_9e868e = array (
			'search' => 'mediaresult',
			'type' => 'item',
			'value' => 'block',
			'mandatory' => 'true',
		);
		$values_17_9e868e['value'] = $blockAttributes['search']['mediaresult']['item'];
		if ($values_17_9e868e['type'] == 'publication date after' || $values_17_9e868e['type'] == 'publication date before') {
			//convert DB format to current language format
			$dt = new CMS_date();
			$dt->setFromDBValue($values_17_9e868e['value']);
			$values_17_9e868e['value'] = $dt->getLocalizedDate($cms_language->getDateFormat());
		}
		$launchSearch_mediaresult = (CMS_polymod_definition_parsing::addSearchCondition($search_mediaresult, $values_17_9e868e)) ? $launchSearch_mediaresult : false;
	} elseif (true == true) {
		//search parameter is mandatory and no value found
		$launchSearch_mediaresult = false;
	}
	//RESULT mediaresult TAG START 18_d3bdce
	...
	if ($results_mediaresult) {
		$object_18_d3bdce = (isset($object[$objectDefinition_mediaresult])) ? $object[$objectDefinition_mediaresult] : ""; //save previous object search if any
		$replace_18_d3bdce = $replace; //save previous replace vars if any
		$count_18_d3bdce = 0;
		$content_18_d3bdce = $content; //save previous content var if any
		$maxPages_18_d3bdce = $search_mediaresult->getMaxPages();
		$maxResults_18_d3bdce = $search_mediaresult->getNumRows();
		foreach ($results_mediaresult as $object[$objectDefinition_mediaresult]) {
			$content = "";
			$replace["atm-search"] = array (
				"{resultid}" 	=> (isset($resultID_mediaresult)) ? $resultID_mediaresult : $object[$objectDefinition_mediaresult]->getID(),
				"{firstresult}" => (!$count_18_d3bdce) ? 1 : 0,
				"{lastresult}" 	=> ($count_18_d3bdce == sizeof($results_mediaresult)-1) ? 1 : 0,
				"{resultcount}" => ($count_18_d3bdce+1),
				"{maxpages}"    => $maxPages_18_d3bdce,
				"{currentpage}" => ($search_mediaresult->getAttribute('page')+1),
				"{maxresults}"  => $maxResults_18_d3bdce,
			);
			//IF TAG START 19_685a8e
			...
			if ($ifcondition) {
				...
				if ($func()) {
					...
					}//IF TAG END 20_3af946
					$content .=" ".$object[2]->getValue('label','')."</a>
					";
					//IF TAG START 21_438ab4
					...
					}//IF TAG END 21_438ab4
				}
			}//IF TAG END 19_685a8e
			//IF TAG START 22_1beaf9
			$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('fileExtension',''))." == 'flv'", $replace);
			if ($ifcondition) {
				$func = create_function("","return (".$ifcondition.");");
				if ($func()) {
					//IF TAG START 23_f0736b
					$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
					if ($ifcondition) {
						$func = create_function("","return (".$ifcondition.");");
						if ($func()) {
					...
					}//IF TAG END 23_f0736b
					//IF TAG START 24_73e342
					$ifcondition = CMS_polymod_definition_parsing::replaceVars("!".CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
					if ($ifcondition) {
						$func = create_function("","return (".$ifcondition.");");
						if ($func()) {
			...
			}//IF TAG END 22_1beaf9
			//IF TAG START 25_0fca05
			...
			if ($ifcondition) {
				...
				if ($func()) {
					...
					}//IF TAG END 26_be436a
				}
			}//IF TAG END 25_0fca05
			//IF TAG START 27_cfd5da
			...
			if ($ifcondition) {
				...
				if ($func()) {
					...
					}//IF TAG END 28_e21fc0
					//IF TAG START 29_ec5188
					$ifcondition = CMS_polymod_definition_parsing::replaceVars("!".CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
					if ($ifcondition) {
						$func = create_function("","return (".$ifcondition.");");
						if ($func()) {
					...
					}//IF TAG END 29_ec5188
					$content .="
					</div>
					";
				}
			}//IF TAG END 27_cfd5da
			$count_18_d3bdce++;
			//do all result vars replacement
			$content_18_d3bdce.= CMS_polymod_definition_parsing::replaceVars($content, $replace);
		}
		$content = $content_18_d3bdce; //retrieve previous content var if any
		$replace = $replace_18_d3bdce; //retrieve previous replace vars if any
		$object[$objectDefinition_mediaresult] = $object_18_d3bdce; //retrieve previous object search if any
	}
	//RESULT mediaresult TAG END 18_d3bdce
	//destroy search and results mediaresult objects
	unset($search_mediaresult);
	unset($results_mediaresult);
	//SEARCH mediaresult TAG END 16_2aad13
