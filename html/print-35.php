<?php //Generated on Tue, 23 Jun 2009 18:05:19 +0200 by Automne (TM) 4.0.0rc1
...
<h2>Principe de gestion des droits</h2>
	...
	<div class="imgRight">
		<?php //Generated by : $Id: print-35.php,v 1.2 2009/06/23 16:00:42 sebastien Exp $
$content = "";
$replace = "";
...
//SEARCH mediaresult TAG START 16_019aec
$objectDefinition_mediaresult = '2';
if (!isset($objectDefinitions[$objectDefinition_mediaresult])) {
	$objectDefinitions[$objectDefinition_mediaresult] = new CMS_poly_object_definition($objectDefinition_mediaresult);
}
//public search ?
$public_16_019aec = isset($public_search) ? $public_search : false;
//get search params
$search_mediaresult = new CMS_object_search($objectDefinitions[$objectDefinition_mediaresult], $public_16_019aec);
$launchSearch_mediaresult = true;
//add search conditions if any
if (isset($blockAttributes['search']['mediaresult']['item'])) {
	$values_17_26b848 = array (
		'search' => 'mediaresult',
		'type' => 'item',
		'value' => 'block',
		'mandatory' => 'true',
	);
	$values_17_26b848['value'] = $blockAttributes['search']['mediaresult']['item'];
	if ($values_17_26b848['type'] == 'publication date after' || $values_17_26b848['type'] == 'publication date before') {
		//convert DB format to current language format
		$dt = new CMS_date();
		$dt->setFromDBValue($values_17_26b848['value']);
		$values_17_26b848['value'] = $dt->getLocalizedDate($cms_language->getDateFormat());
	}
	$launchSearch_mediaresult = (CMS_polymod_definition_parsing::addSearchCondition($search_mediaresult, $values_17_26b848)) ? $launchSearch_mediaresult : false;
} elseif (true == true) {
	//search parameter is mandatory and no value found
	$launchSearch_mediaresult = false;
}
//RESULT mediaresult TAG START 18_e661fc
...
if ($results_mediaresult) {
	$object_18_e661fc = $object[$objectDefinition_mediaresult]; //save previous object search if any
	$replace_18_e661fc = $replace; //save previous replace vars if any
	$count_18_e661fc = 0;
	$content_18_e661fc = $content; //save previous content var if any
	$maxPages_18_e661fc = $search_mediaresult->getMaxPages();
	$maxResults_18_e661fc = $search_mediaresult->getNumRows();
	foreach ($results_mediaresult as $object[$objectDefinition_mediaresult]) {
		$content = "";
		$replace["atm-search"] = array (
			"{resultid}" 	=> (isset($resultID_mediaresult)) ? $resultID_mediaresult : $object[$objectDefinition_mediaresult]->getID(),
			"{firstresult}" => (!$count_18_e661fc) ? 1 : 0,
			"{lastresult}" 	=> ($count_18_e661fc == sizeof($results_mediaresult)-1) ? 1 : 0,
			"{resultcount}" => ($count_18_e661fc+1),
			"{maxpages}"    => $maxPages_18_e661fc,
			"{currentpage}" => ($search_mediaresult->getAttribute('page')+1),
			"{maxresults}"  => $maxResults_18_e661fc,
		);
		//IF TAG START 19_542163
		...
		if ($ifcondition) {
			...
			if ($func()) {
				...
				}//IF TAG END 20_30bdfe
				$content .=" ".$object[2]->getValue('label','')."</a>
				";
				//IF TAG START 21_9e3fb0
				...
				}//IF TAG END 21_9e3fb0
			}
		}//IF TAG END 19_542163
		//IF TAG START 22_f8faaf
		$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('fileExtension',''))." == 'flv'", $replace);
		if ($ifcondition) {
			$func = create_function("","return (".$ifcondition.");");
			if ($func()) {
				//IF TAG START 23_482e75
				$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
				if ($ifcondition) {
					$func = create_function("","return (".$ifcondition.");");
					if ($func()) {
				...
				}//IF TAG END 23_482e75
				//IF TAG START 24_1b4c47
				$ifcondition = CMS_polymod_definition_parsing::replaceVars("!".CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
				if ($ifcondition) {
					$func = create_function("","return (".$ifcondition.");");
					if ($func()) {
		...
		}//IF TAG END 22_f8faaf
		//IF TAG START 25_2666d6
		...
		if ($ifcondition) {
			...
			if ($func()) {
				...
				}//IF TAG END 26_e52ca3
			}
		}//IF TAG END 25_2666d6
		//IF TAG START 27_b75469
		...
		if ($ifcondition) {
			...
			if ($func()) {
				...
				}//IF TAG END 28_9a57bc
				//IF TAG START 29_4d513c
				$ifcondition = CMS_polymod_definition_parsing::replaceVars("!".CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
				if ($ifcondition) {
					$func = create_function("","return (".$ifcondition.");");
					if ($func()) {
		...
		}//IF TAG END 27_b75469
		$count_18_e661fc++;
		//do all result vars replacement
		$content_18_e661fc.= CMS_polymod_definition_parsing::replaceVars($content, $replace);
	}
	$content = $content_18_e661fc; //retrieve previous content var if any
	$replace = $replace_18_e661fc; //retrieve previous replace vars if any
	$object[$objectDefinition_mediaresult] = $object_18_e661fc; //retrieve previous object search if any
}
//RESULT mediaresult TAG END 18_e661fc
//destroy search and results mediaresult objects
unset($search_mediaresult);
unset($results_mediaresult);
//SEARCH mediaresult TAG END 16_019aec
