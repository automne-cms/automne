...
<h2>Principe de gestion des droits</h2>
	...
	<div class="imgRight">
		<?php //Generated by : $Id: print-35.php,v 1.4 2009/10/28 16:31:36 sebastien Exp $
$content = "";
$replace = "";
...
//SEARCH mediaresult TAG START 16_a24465
$objectDefinition_mediaresult = '2';
if (!isset($objectDefinitions[$objectDefinition_mediaresult])) {
	$objectDefinitions[$objectDefinition_mediaresult] = new CMS_poly_object_definition($objectDefinition_mediaresult);
}
//public search ?
$public_16_a24465 = isset($public_search) ? $public_search : false;
//get search params
$search_mediaresult = new CMS_object_search($objectDefinitions[$objectDefinition_mediaresult], $public_16_a24465);
$launchSearch_mediaresult = true;
//add search conditions if any
if (isset($blockAttributes['search']['mediaresult']['item'])) {
	$values_17_d60de3 = array (
		'search' => 'mediaresult',
		'type' => 'item',
		'value' => 'block',
		'mandatory' => 'true',
	);
	$values_17_d60de3['value'] = $blockAttributes['search']['mediaresult']['item'];
	if ($values_17_d60de3['type'] == 'publication date after' || $values_17_d60de3['type'] == 'publication date before') {
		//convert DB format to current language format
		$dt = new CMS_date();
		$dt->setFromDBValue($values_17_d60de3['value']);
		$values_17_d60de3['value'] = $dt->getLocalizedDate($cms_language->getDateFormat());
	}
	$launchSearch_mediaresult = (CMS_polymod_definition_parsing::addSearchCondition($search_mediaresult, $values_17_d60de3)) ? $launchSearch_mediaresult : false;
} elseif (true == true) {
	//search parameter is mandatory and no value found
	$launchSearch_mediaresult = false;
}
//RESULT mediaresult TAG START 18_a0a2fa
...
if ($results_mediaresult) {
	$object_18_a0a2fa = (isset($object[$objectDefinition_mediaresult])) ? $object[$objectDefinition_mediaresult] : ""; //save previous object search if any
	$replace_18_a0a2fa = $replace; //save previous replace vars if any
	$count_18_a0a2fa = 0;
	$content_18_a0a2fa = $content; //save previous content var if any
	$maxPages_18_a0a2fa = $search_mediaresult->getMaxPages();
	$maxResults_18_a0a2fa = $search_mediaresult->getNumRows();
	foreach ($results_mediaresult as $object[$objectDefinition_mediaresult]) {
		$content = "";
		$replace["atm-search"] = array (
			"{resultid}" 	=> (isset($resultID_mediaresult)) ? $resultID_mediaresult : $object[$objectDefinition_mediaresult]->getID(),
			"{firstresult}" => (!$count_18_a0a2fa) ? 1 : 0,
			"{lastresult}" 	=> ($count_18_a0a2fa == sizeof($results_mediaresult)-1) ? 1 : 0,
			"{resultcount}" => ($count_18_a0a2fa+1),
			"{maxpages}"    => $maxPages_18_a0a2fa,
			"{currentpage}" => ($search_mediaresult->getAttribute('page')+1),
			"{maxresults}"  => $maxResults_18_a0a2fa,
		);
		//IF TAG START 19_08afdc
		...
		if ($ifcondition) {
			...
			if ($func()) {
				...
				}//IF TAG END 20_585885
				$content .=" ".$object[2]->getValue('label','')."</a>
				";
				//IF TAG START 21_1cd59f
				...
				}//IF TAG END 21_1cd59f
			}
		}//IF TAG END 19_08afdc
		//IF TAG START 22_33c430
		$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('fileExtension',''))." == 'flv'", $replace);
		if ($ifcondition) {
			$func = create_function("","return (".$ifcondition.");");
			if ($func()) {
				//IF TAG START 23_86b388
				$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
				if ($ifcondition) {
					$func = create_function("","return (".$ifcondition.");");
					if ($func()) {
				...
				}//IF TAG END 23_86b388
				//IF TAG START 24_4aed04
				$ifcondition = CMS_polymod_definition_parsing::replaceVars("!".CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
				if ($ifcondition) {
					$func = create_function("","return (".$ifcondition.");");
					if ($func()) {
		...
		}//IF TAG END 22_33c430
		//IF TAG START 25_7602da
		...
		if ($ifcondition) {
			...
			if ($func()) {
				...
				}//IF TAG END 26_cf4daf
			}
		}//IF TAG END 25_7602da
		//IF TAG START 27_b0bf9e
		...
		if ($ifcondition) {
			...
			if ($func()) {
				...
				}//IF TAG END 28_b6d509
				//IF TAG START 29_64304e
				$ifcondition = CMS_polymod_definition_parsing::replaceVars("!".CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
				if ($ifcondition) {
					$func = create_function("","return (".$ifcondition.");");
					if ($func()) {
		...
		}//IF TAG END 27_b0bf9e
		$count_18_a0a2fa++;
		//do all result vars replacement
		$content_18_a0a2fa.= CMS_polymod_definition_parsing::replaceVars($content, $replace);
	}
	$content = $content_18_a0a2fa; //retrieve previous content var if any
	$replace = $replace_18_a0a2fa; //retrieve previous replace vars if any
	$object[$objectDefinition_mediaresult] = $object_18_a0a2fa; //retrieve previous object search if any
}
//RESULT mediaresult TAG END 18_a0a2fa
//destroy search and results mediaresult objects
unset($search_mediaresult);
unset($results_mediaresult);
//SEARCH mediaresult TAG END 16_a24465
