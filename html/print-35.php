...
?><?php if ($cms_user->hasPageClearance(35, CLEARANCE_PAGE_VIEW)) {
echo '
...
<h2>Principe de gestion des droits</h2>
	...
	<div class="imgRight">
		<?php //Generated by : $Id: print-35.php,v 1.6 2010/01/18 15:20:10 sebastien Exp $
if(!APPLICATION_ENFORCES_ACCESS_CONTROL || (isset($cms_user) && is_a($cms_user, 'CMS_profile_user') && $cms_user->hasModuleClearance('pmedia', CLEARANCE_MODULE_VIEW))){
	...
	//SEARCH mediaresult TAG START 16_5f2070
	$objectDefinition_mediaresult = '2';
	if (!isset($objectDefinitions[$objectDefinition_mediaresult])) {
		$objectDefinitions[$objectDefinition_mediaresult] = new CMS_poly_object_definition($objectDefinition_mediaresult);
	}
	//public search ?
	$public_16_5f2070 = isset($public_search) ? $public_search : false;
	//get search params
	$search_mediaresult = new CMS_object_search($objectDefinitions[$objectDefinition_mediaresult], $public_16_5f2070);
	$launchSearch_mediaresult = true;
	//add search conditions if any
	if (isset($blockAttributes['search']['mediaresult']['item'])) {
		$values_17_54c225 = array (
			'search' => 'mediaresult',
			'type' => 'item',
			'value' => 'block',
			'mandatory' => 'true',
		);
		$values_17_54c225['value'] = $blockAttributes['search']['mediaresult']['item'];
		if ($values_17_54c225['type'] == 'publication date after' || $values_17_54c225['type'] == 'publication date before') {
			//convert DB format to current language format
			$dt = new CMS_date();
			$dt->setFromDBValue($values_17_54c225['value']);
			$values_17_54c225['value'] = $dt->getLocalizedDate($cms_language->getDateFormat());
		}
		$launchSearch_mediaresult = (CMS_polymod_definition_parsing::addSearchCondition($search_mediaresult, $values_17_54c225)) ? $launchSearch_mediaresult : false;
	} elseif (true == true) {
		//search parameter is mandatory and no value found
		$launchSearch_mediaresult = false;
	}
	//RESULT mediaresult TAG START 18_9572f7
	...
	if ($results_mediaresult) {
		$object_18_9572f7 = (isset($object[$objectDefinition_mediaresult])) ? $object[$objectDefinition_mediaresult] : ""; //save previous object search if any
		$replace_18_9572f7 = $replace; //save previous replace vars if any
		$count_18_9572f7 = 0;
		$content_18_9572f7 = $content; //save previous content var if any
		$maxPages_18_9572f7 = $search_mediaresult->getMaxPages();
		$maxResults_18_9572f7 = $search_mediaresult->getNumRows();
		foreach ($results_mediaresult as $object[$objectDefinition_mediaresult]) {
			$content = "";
			$replace["atm-search"] = array (
				"{resultid}" 	=> (isset($resultID_mediaresult)) ? $resultID_mediaresult : $object[$objectDefinition_mediaresult]->getID(),
				"{firstresult}" => (!$count_18_9572f7) ? 1 : 0,
				"{lastresult}" 	=> ($count_18_9572f7 == sizeof($results_mediaresult)-1) ? 1 : 0,
				"{resultcount}" => ($count_18_9572f7+1),
				"{maxpages}"    => $maxPages_18_9572f7,
				"{currentpage}" => ($search_mediaresult->getAttribute('page')+1),
				"{maxresults}"  => $maxResults_18_9572f7,
			);
			//IF TAG START 19_9f944e
			...
			if ($ifcondition) {
				...
				if ($func()) {
					...
					}//IF TAG END 20_47b0cb
					$content .=" ".$object[2]->getValue('label','')."</a>
					";
					//IF TAG START 21_00a517
					...
					}//IF TAG END 21_00a517
				}
			}//IF TAG END 19_9f944e
			//IF TAG START 22_6cc720
			$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('fileExtension',''))." == 'flv'", $replace);
			if ($ifcondition) {
				$func = create_function("","return (".$ifcondition.");");
				if ($func()) {
					//IF TAG START 23_892254
					$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
					if ($ifcondition) {
						$func = create_function("","return (".$ifcondition.");");
						if ($func()) {
					...
					}//IF TAG END 23_892254
					//IF TAG START 24_f3d649
					$ifcondition = CMS_polymod_definition_parsing::replaceVars("!".CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
					if ($ifcondition) {
						$func = create_function("","return (".$ifcondition.");");
						if ($func()) {
			...
			}//IF TAG END 22_6cc720
			//IF TAG START 25_79da32
			...
			if ($ifcondition) {
				...
				if ($func()) {
					...
					}//IF TAG END 26_348201
				}
			}//IF TAG END 25_79da32
			//IF TAG START 27_1e8ce8
			...
			if ($ifcondition) {
				...
				if ($func()) {
					...
					}//IF TAG END 28_6d78f4
					//IF TAG START 29_4cb898
					$ifcondition = CMS_polymod_definition_parsing::replaceVars("!".CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
					if ($ifcondition) {
						$func = create_function("","return (".$ifcondition.");");
						if ($func()) {
					...
					}//IF TAG END 29_4cb898
					$content .="
					</div>
					";
				}
			}//IF TAG END 27_1e8ce8
			$count_18_9572f7++;
			//do all result vars replacement
			$content_18_9572f7.= CMS_polymod_definition_parsing::replaceVars($content, $replace);
		}
		$content = $content_18_9572f7; //retrieve previous content var if any
		$replace = $replace_18_9572f7; //retrieve previous replace vars if any
		$object[$objectDefinition_mediaresult] = $object_18_9572f7; //retrieve previous object search if any
	}
	//RESULT mediaresult TAG END 18_9572f7
	//destroy search and results mediaresult objects
	unset($search_mediaresult);
	unset($results_mediaresult);
	//SEARCH mediaresult TAG END 16_5f2070
