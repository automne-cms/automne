<?php //Generated on Mon, 03 May 2010 09:40:24 +0200 by Automne (TM) 4.0.1
require_once($_SERVER["DOCUMENT_ROOT"]."/cms_rc_frontend.php");
if (!isset($cms_page_included) && !$_POST && !$_GET) {
	CMS_view::redirect('http://acezar.401/web/demo/print-35-gestion-des-droits.php', true, 301);
}
 ?>
...
if(!APPLICATION_ENFORCES_ACCESS_CONTROL || (isset($cms_user) && is_a($cms_user, 'CMS_profile_user') && $cms_user->hasModuleClearance('pmedia', CLEARANCE_MODULE_VIEW))){
	...
	//SEARCH mediaresult TAG START 16_b1ba6f
	$objectDefinition_mediaresult = '2';
	if (!isset($objectDefinitions[$objectDefinition_mediaresult])) {
		$objectDefinitions[$objectDefinition_mediaresult] = new CMS_poly_object_definition($objectDefinition_mediaresult);
	}
	//public search ?
	$public_16_b1ba6f = isset($public_search) ? $public_search : false;
	//get search params
	$search_mediaresult = new CMS_object_search($objectDefinitions[$objectDefinition_mediaresult], $public_16_b1ba6f);
	$launchSearch_mediaresult = true;
	//add search conditions if any
	if (isset($blockAttributes['search']['mediaresult']['item'])) {
		$values_17_f75eb3 = array (
			'search' => 'mediaresult',
			'type' => 'item',
			'value' => 'block',
			'mandatory' => 'true',
		);
		$values_17_f75eb3['value'] = $blockAttributes['search']['mediaresult']['item'];
		if ($values_17_f75eb3['type'] == 'publication date after' || $values_17_f75eb3['type'] == 'publication date before') {
			//convert DB format to current language format
			$dt = new CMS_date();
			$dt->setFromDBValue($values_17_f75eb3['value']);
			$values_17_f75eb3['value'] = $dt->getLocalizedDate($cms_language->getDateFormat());
		}
		$launchSearch_mediaresult = (CMS_polymod_definition_parsing::addSearchCondition($search_mediaresult, $values_17_f75eb3)) ? $launchSearch_mediaresult : false;
	} elseif (true == true) {
		//search parameter is mandatory and no value found
		$launchSearch_mediaresult = false;
	}
	//RESULT mediaresult TAG START 18_2daf36
	...
	if ($results_mediaresult) {
		$object_18_2daf36 = (isset($object[$objectDefinition_mediaresult])) ? $object[$objectDefinition_mediaresult] : ""; //save previous object search if any
		$replace_18_2daf36 = $replace; //save previous replace vars if any
		$count_18_2daf36 = 0;
		$content_18_2daf36 = $content; //save previous content var if any
		$maxPages_18_2daf36 = $search_mediaresult->getMaxPages();
		$maxResults_18_2daf36 = $search_mediaresult->getNumRows();
		foreach ($results_mediaresult as $object[$objectDefinition_mediaresult]) {
			$content = "";
			$replace["atm-search"] = array (
				"{resultid}" 	=> (isset($resultID_mediaresult)) ? $resultID_mediaresult : $object[$objectDefinition_mediaresult]->getID(),
				"{firstresult}" => (!$count_18_2daf36) ? 1 : 0,
				"{lastresult}" 	=> ($count_18_2daf36 == sizeof($results_mediaresult)-1) ? 1 : 0,
				"{resultcount}" => ($count_18_2daf36+1),
				"{maxpages}"    => $maxPages_18_2daf36,
				"{currentpage}" => ($search_mediaresult->getAttribute('page')+1),
				"{maxresults}"  => $maxResults_18_2daf36,
			);
			//IF TAG START 19_6232ce
			...
			if ($ifcondition) {
				...
				if ($func()) {
					...
					}//IF TAG END 20_0d3a92
					$content .=" ".$object[2]->getValue('label','')."</a>
					";
					//IF TAG START 21_a712e4
					...
					}//IF TAG END 21_a712e4
				}
			}//IF TAG END 19_6232ce
			//IF TAG START 22_bf9928
			$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('fileExtension',''))." == 'flv'", $replace);
			if ($ifcondition) {
				$func = create_function("","return (".$ifcondition.");");
				if ($func()) {
					//IF TAG START 23_b65016
					$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
					if ($ifcondition) {
						$func = create_function("","return (".$ifcondition.");");
						if ($func()) {
					...
					}//IF TAG END 23_b65016
					//IF TAG START 24_1ce38a
					$ifcondition = CMS_polymod_definition_parsing::replaceVars("!".CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
					if ($ifcondition) {
						$func = create_function("","return (".$ifcondition.");");
						if ($func()) {
			...
			}//IF TAG END 22_bf9928
			//IF TAG START 25_b19fda
			...
			if ($ifcondition) {
				...
				if ($func()) {
					...
					}//IF TAG END 26_577900
				}
			}//IF TAG END 25_b19fda
			//IF TAG START 27_69e538
			...
			if ($ifcondition) {
				...
				if ($func()) {
					...
					}//IF TAG END 28_5738e1
					//IF TAG START 29_723a6d
					$ifcondition = CMS_polymod_definition_parsing::replaceVars("!".CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
					if ($ifcondition) {
						$func = create_function("","return (".$ifcondition.");");
						if ($func()) {
					...
					}//IF TAG END 29_723a6d
					$content .="
					</div>
					";
				}
			}//IF TAG END 27_69e538
			$count_18_2daf36++;
			//do all result vars replacement
			$content_18_2daf36.= CMS_polymod_definition_parsing::replaceVars($content, $replace);
		}
		$content = $content_18_2daf36; //retrieve previous content var if any
		$replace = $replace_18_2daf36; //retrieve previous replace vars if any
		$object[$objectDefinition_mediaresult] = $object_18_2daf36; //retrieve previous object search if any
	}
	//RESULT mediaresult TAG END 18_2daf36
	//destroy search and results mediaresult objects
	unset($search_mediaresult);
	unset($results_mediaresult);
	//SEARCH mediaresult TAG END 16_b1ba6f
...
<div align="center">
	<small>
				...
				Page  "Gestion des droits" (http://acezar.401/web/demo/35-gestion-des-droits.php)
				<br />
