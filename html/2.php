<?php //Generated on Mon, 18 Jan 2010 16:12:27 +0100 by Automne (TM) 4.0.0
require_once($_SERVER["DOCUMENT_ROOT"]."/cms_rc_frontend.php");
if (!isset($cms_page_included) && !$_POST && !$_GET) {
	CMS_view::redirect('http://127.0.0.1/web/demo/2-accueil.php', true, 301);
}
 ?><?php if (!is_object($cms_user) || !$cms_user->hasPageClearance(2, CLEARANCE_PAGE_VIEW)) {
...
echo '<li class="CMS_lvl2"><a class="CMS_lvl2" href="http://127.0.0.1/web/demo/3-presentation.php">Présentation</a></li>';
}
?><?php if ($cms_user->hasPageClearance(24, CLEARANCE_PAGE_VIEW)) {
echo '<li class="CMS_lvl2"><a class="CMS_lvl2" href="http://127.0.0.1/web/demo/24-documentation.php">Fonctionnalités</a></li>';
}
?><?php if ($cms_user->hasPageClearance(31, CLEARANCE_PAGE_VIEW)) {
echo '<li class="CMS_lvl2"><a class="CMS_lvl2" href="http://127.0.0.1/web/demo/31-exemples-de-modules.php">Exemples de modules</a></li>';
}
?>
				</ul>
			</div>
...
<p><strong>Automne est votre solution</strong> si vous recherchez un outil de gestion de contenu performant et &eacute;volutif. </p><p>Un outil permettant autonomie et contr&ocirc;le &eacute;ditorial.</p><p>Que votre contenu soit statique ou dynamique avec une gestion en bases de donn&eacute;es, Automne facilite la communication et les &eacute;changes <strong>sans contraintes techniques.<br /></strong></p>

				</div>
				<div id="secondCol">
					
	<?php //Generated by : $Id: 2.php,v 1.11 2010/01/18 15:20:08 sebastien Exp $
if(!APPLICATION_ENFORCES_ACCESS_CONTROL || (isset($cms_user) && is_a($cms_user, 'CMS_profile_user') && $cms_user->hasModuleClearance('pnews', CLEARANCE_MODULE_VIEW))){
	...
	//SEARCH lastNews TAG START 2_a34f9b
	...
	//public search ?
	$public_2_a34f9b = isset($public_search) ? $public_search : false;
	//get search params
	$search_lastNews = new CMS_object_search($objectDefinitions[$objectDefinition_lastNews], $public_2_a34f9b);
	...
	//RESULT lastNews TAG START 3_d15ee7
	...
	if ($results_lastNews) {
		$object_3_d15ee7 = (isset($object[$objectDefinition_lastNews])) ? $object[$objectDefinition_lastNews] : ""; //save previous object search if any
		$replace_3_d15ee7 = $replace; //save previous replace vars if any
		$count_3_d15ee7 = 0;
		$content_3_d15ee7 = $content; //save previous content var if any
		$maxPages_3_d15ee7 = $search_lastNews->getMaxPages();
		$maxResults_3_d15ee7 = $search_lastNews->getNumRows();
		foreach ($results_lastNews as $object[$objectDefinition_lastNews]) {
			...
			$replace["atm-search"] = array (
				...
				"{firstresult}" => (!$count_3_d15ee7) ? 1 : 0,
				"{lastresult}" 	=> ($count_3_d15ee7 == sizeof($results_lastNews)-1) ? 1 : 0,
				"{resultcount}" => ($count_3_d15ee7+1),
				"{maxpages}"    => $maxPages_3_d15ee7,
				"{currentpage}" => ($search_lastNews->getAttribute('page')+1),
				"{maxresults}"  => $maxResults_3_d15ee7,
			);
			$content .="
			<div class=\"lastNews\">
			<div class=\"newsTop\">
			...
			";
			//FUNCTION TAG START 6_1b668e
			$parameters_6_1b668e = array ('selected' => CMS_polymod_definition_parsing::replaceVars("3", $replace),);
			$object_6_1b668e = &$object[1];
			if (method_exists($object_6_1b668e, "rss")) {
				$content .= CMS_polymod_definition_parsing::replaceVars($object_6_1b668e->rss($parameters_6_1b668e, array (
					0 =>
					array (
						'textnode' => '
						',
				...
				CMS_grandFather::raiseError("Malformed atm-function tag : can't found method rss on object : ".get_class($object_6_1b668e));
			}
			//FUNCTION TAG END 6_1b668e
			$content .="
			</div>
			</div>
			</div>
			...
			$content_3_d15ee7.= CMS_polymod_definition_parsing::replaceVars($content, $replace);
		}
		$content = $content_3_d15ee7; //retrieve previous content var if any
		$replace = $replace_3_d15ee7; //retrieve previous replace vars if any
		$object[$objectDefinition_lastNews] = $object_3_d15ee7; //retrieve previous object search if any
	}
	//RESULT lastNews TAG END 3_d15ee7
	//destroy search and results lastNews objects
	unset($search_lastNews);
	unset($results_lastNews);
	//SEARCH lastNews TAG END 2_a34f9b
