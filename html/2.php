<?php //Generated on Tue, 09 Mar 2010 18:25:19 +0100 by Automne (TM) 4.0.1
...
if(!APPLICATION_ENFORCES_ACCESS_CONTROL || (isset($cms_user) && is_a($cms_user, 'CMS_profile_user') && $cms_user->hasModuleClearance('pnews', CLEARANCE_MODULE_VIEW))){
	...
	//SEARCH lastNews TAG START 2_64ea54
	...
	//public search ?
	$public_2_64ea54 = isset($public_search) ? $public_search : false;
	//get search params
	$search_lastNews = new CMS_object_search($objectDefinitions[$objectDefinition_lastNews], $public_2_64ea54);
	...
	//RESULT lastNews TAG START 3_466c01
	...
	if ($results_lastNews) {
		$object_3_466c01 = (isset($object[$objectDefinition_lastNews])) ? $object[$objectDefinition_lastNews] : ""; //save previous object search if any
		$replace_3_466c01 = $replace; //save previous replace vars if any
		$count_3_466c01 = 0;
		$content_3_466c01 = $content; //save previous content var if any
		$maxPages_3_466c01 = $search_lastNews->getMaxPages();
		$maxResults_3_466c01 = $search_lastNews->getNumRows();
		foreach ($results_lastNews as $object[$objectDefinition_lastNews]) {
			...
			$replace["atm-search"] = array (
				...
				"{firstresult}" => (!$count_3_466c01) ? 1 : 0,
				"{lastresult}" 	=> ($count_3_466c01 == sizeof($results_lastNews)-1) ? 1 : 0,
				"{resultcount}" => ($count_3_466c01+1),
				"{maxpages}"    => $maxPages_3_466c01,
				"{currentpage}" => ($search_lastNews->getAttribute('page')+1),
				"{maxresults}"  => $maxResults_3_466c01,
			);
			$content .="
			<div class=\"lastNews\">
			<div class=\"newsTop\">
			...
			";
			//FUNCTION TAG START 6_19144c
			$parameters_6_19144c = array ('selected' => CMS_polymod_definition_parsing::replaceVars("3", $replace),);
			$object_6_19144c = &$object[1];
			if (method_exists($object_6_19144c, "rss")) {
				$content .= CMS_polymod_definition_parsing::replaceVars($object_6_19144c->rss($parameters_6_19144c, array (
					0 =>
					array (
						'textnode' => '
						',
				...
				CMS_grandFather::raiseError("Malformed atm-function tag : can't found method rss on object : ".get_class($object_6_19144c));
			}
			//FUNCTION TAG END 6_19144c
			$content .="
			</div>
			</div>
			</div>
			...
			$content_3_466c01.= CMS_polymod_definition_parsing::replaceVars($content, $replace);
		}
		$content = $content_3_466c01; //retrieve previous content var if any
		$replace = $replace_3_466c01; //retrieve previous replace vars if any
		$object[$objectDefinition_lastNews] = $object_3_466c01; //retrieve previous object search if any
	}
	//RESULT lastNews TAG END 3_466c01
	//destroy search and results lastNews objects
	unset($search_lastNews);
	unset($results_lastNews);
	//SEARCH lastNews TAG END 2_64ea54
