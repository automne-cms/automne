...
<div class="text"><p><strong>Automne est votre solution</strong> si vous recherchez un outil de gestion de contenu performant et &eacute;volutif. </p><p>Un outil permettant autonomie et contr&ocirc;le &eacute;ditorial.</p><p>Que votre contenu soit statique ou dynamique avec une gestion en bases de donn&eacute;es, Automne facilite la communication et les &eacute;changes <strong>sans contraintes techniques.<br /></strong></p></div>
	...
	<?php //Generated by : $Id: 2.php,v 1.10 2009/11/17 16:04:27 sebastien Exp $
if(!APPLICATION_ENFORCES_ACCESS_CONTROL || (isset($cms_user) && is_a($cms_user, 'CMS_profile_user') && $cms_user->hasModuleClearance('pnews', CLEARANCE_MODULE_VIEW))){
	...
	//SEARCH lastNews TAG START 2_39afaa
	...
	//public search ?
	$public_2_39afaa = isset($public_search) ? $public_search : false;
	//get search params
	$search_lastNews = new CMS_object_search($objectDefinitions[$objectDefinition_lastNews], $public_2_39afaa);
	...
	//RESULT lastNews TAG START 3_63a08d
	...
	if ($results_lastNews) {
		$object_3_63a08d = (isset($object[$objectDefinition_lastNews])) ? $object[$objectDefinition_lastNews] : ""; //save previous object search if any
		$replace_3_63a08d = $replace; //save previous replace vars if any
		$count_3_63a08d = 0;
		$content_3_63a08d = $content; //save previous content var if any
		$maxPages_3_63a08d = $search_lastNews->getMaxPages();
		$maxResults_3_63a08d = $search_lastNews->getNumRows();
		foreach ($results_lastNews as $object[$objectDefinition_lastNews]) {
			...
			$replace["atm-search"] = array (
				...
				"{firstresult}" => (!$count_3_63a08d) ? 1 : 0,
				"{lastresult}" 	=> ($count_3_63a08d == sizeof($results_lastNews)-1) ? 1 : 0,
				"{resultcount}" => ($count_3_63a08d+1),
				"{maxpages}"    => $maxPages_3_63a08d,
				"{currentpage}" => ($search_lastNews->getAttribute('page')+1),
				"{maxresults}"  => $maxResults_3_63a08d,
			);
			$content .="
			<div class=\"lastNews\">
			<div class=\"newsTop\">
			...
			";
			//FUNCTION TAG START 6_c5ca39
			$parameters_6_c5ca39 = array ('selected' => CMS_polymod_definition_parsing::replaceVars("3", $replace),);
			$object_6_c5ca39 = &$object[1];
			if (method_exists($object_6_c5ca39, "rss")) {
				$content .= CMS_polymod_definition_parsing::replaceVars($object_6_c5ca39->rss($parameters_6_c5ca39, array (
					0 =>
					array (
						'textnode' => '
						',
				...
				CMS_grandFather::raiseError("Malformed atm-function tag : can't found method rss on object : ".get_class($object_6_c5ca39));
			}
			//FUNCTION TAG END 6_c5ca39
			$content .="
			</div>
			</div>
			</div>
			...
			$content_3_63a08d.= CMS_polymod_definition_parsing::replaceVars($content, $replace);
		}
		$content = $content_3_63a08d; //retrieve previous content var if any
		$replace = $replace_3_63a08d; //retrieve previous replace vars if any
		$object[$objectDefinition_lastNews] = $object_3_63a08d; //retrieve previous object search if any
	}
	//RESULT lastNews TAG END 3_63a08d
	//destroy search and results lastNews objects
	unset($search_lastNews);
	unset($results_lastNews);
	//SEARCH lastNews TAG END 2_39afaa
