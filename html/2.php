		...
		<div id="container">
			...
			<div id="menuLeft">
				<div id="menuLeftBottom">
					<ul class="CMS_lvl2">
						<li class="CMS_lvl2"><a class="CMS_lvl2" href="http://127.0.0.1/web/demo/3-presentation.php">Présentation</a></li>
<li class="CMS_lvl2"><a class="CMS_lvl2" href="http://127.0.0.1/web/demo/24-documentation.php">Fonctionnalités</a></li>
<li class="CMS_lvl2"><a class="CMS_lvl2" href="http://127.0.0.1/web/demo/31-exemples-de-modules.php">Exemples de modules</a></li>
					</ul>
				</div>
			</div>
			<div id="content">
...
<div class="text"><p><strong>Automne est votre solution</strong> si vous recherchez un outil de gestion de contenu performant et &eacute;volutif. </p><p>Un outil permettant autonomie et contr&ocirc;le &eacute;ditorial.</p><p>Que votre contenu soit statique ou dynamique avec une gestion en bases de donn&eacute;es, Automne facilite la communication et les &eacute;changes <strong>sans contraintes techniques.<br /></strong></p></div>
	...
	<?php //Generated by : $Id: 2.php,v 1.9 2009/10/28 16:31:33 sebastien Exp $
$content = "";
$replace = "";
...
//SEARCH lastNews TAG START 2_582ab7
...
//public search ?
$public_2_582ab7 = isset($public_search) ? $public_search : false;
//get search params
$search_lastNews = new CMS_object_search($objectDefinitions[$objectDefinition_lastNews], $public_2_582ab7);
...
//RESULT lastNews TAG START 3_67768c
...
if ($results_lastNews) {
	$object_3_67768c = (isset($object[$objectDefinition_lastNews])) ? $object[$objectDefinition_lastNews] : ""; //save previous object search if any
	$replace_3_67768c = $replace; //save previous replace vars if any
	$count_3_67768c = 0;
	$content_3_67768c = $content; //save previous content var if any
	$maxPages_3_67768c = $search_lastNews->getMaxPages();
	$maxResults_3_67768c = $search_lastNews->getNumRows();
	foreach ($results_lastNews as $object[$objectDefinition_lastNews]) {
		...
		$replace["atm-search"] = array (
			...
			"{firstresult}" => (!$count_3_67768c) ? 1 : 0,
			"{lastresult}" 	=> ($count_3_67768c == sizeof($results_lastNews)-1) ? 1 : 0,
			"{resultcount}" => ($count_3_67768c+1),
			"{maxpages}"    => $maxPages_3_67768c,
			"{currentpage}" => ($search_lastNews->getAttribute('page')+1),
			"{maxresults}"  => $maxResults_3_67768c,
		);
		$content .="
		<div class=\"lastNews\">
		<div class=\"newsTop\">
		...
		";
		//FUNCTION TAG START 6_7394c0
		$parameters_6_7394c0 = array ('selected' => CMS_polymod_definition_parsing::replaceVars("3", $replace),);
		$object_6_7394c0 = &$object[1];
		if (method_exists($object_6_7394c0, "rss")) {
			$content .= CMS_polymod_definition_parsing::replaceVars($object_6_7394c0->rss($parameters_6_7394c0, array (
				0 =>
				array (
					'textnode' => '
					',
			...
			CMS_grandFather::raiseError("Malformed atm-function tag : can't found method rss on object : ".get_class($object_6_7394c0));
		}
		//FUNCTION TAG END 6_7394c0
		$content .="
		</div>
		</div>
		</div>
		...
		$content_3_67768c.= CMS_polymod_definition_parsing::replaceVars($content, $replace);
	}
	$content = $content_3_67768c; //retrieve previous content var if any
	$replace = $replace_3_67768c; //retrieve previous replace vars if any
	$object[$objectDefinition_lastNews] = $object_3_67768c; //retrieve previous object search if any
}
//RESULT lastNews TAG END 3_67768c
//destroy search and results lastNews objects
unset($search_lastNews);
unset($results_lastNews);
//SEARCH lastNews TAG END 2_582ab7
