...
if (!isset($cms_page_included) && !$_POST && !$_GET) {
	CMS_view::redirect('http://automne4.trunk/web/demo/2-accueil.php', true, 301);
}
 ?>
...
if(!APPLICATION_ENFORCES_ACCESS_CONTROL || (isset($cms_user) && is_a($cms_user, 'CMS_profile_user') && $cms_user->hasModuleClearance('pnews', CLEARANCE_MODULE_VIEW))){
	...
	//SEARCH lastNews TAG START 2_96f644
	...
	//public search ?
	$public_2_96f644 = isset($public_search) ? $public_search : false;
	//get search params
	$search_lastNews = new CMS_object_search($objectDefinitions[$objectDefinition_lastNews], $public_2_96f644);
	...
	//RESULT lastNews TAG START 3_5e117e
	...
	if ($results_lastNews) {
		$object_3_5e117e = (isset($object[$objectDefinition_lastNews])) ? $object[$objectDefinition_lastNews] : ""; //save previous object search if any
		$replace_3_5e117e = $replace; //save previous replace vars if any
		$count_3_5e117e = 0;
		$content_3_5e117e = $content; //save previous content var if any
		$maxPages_3_5e117e = $search_lastNews->getMaxPages();
		$maxResults_3_5e117e = $search_lastNews->getNumRows();
		foreach ($results_lastNews as $object[$objectDefinition_lastNews]) {
			...
			$replace["atm-search"] = array (
				...
				"{firstresult}" => (!$count_3_5e117e) ? 1 : 0,
				"{lastresult}" 	=> ($count_3_5e117e == sizeof($results_lastNews)-1) ? 1 : 0,
				"{resultcount}" => ($count_3_5e117e+1),
				"{maxpages}"    => $maxPages_3_5e117e,
				"{currentpage}" => ($search_lastNews->getAttribute('page')+1),
				"{maxresults}"  => $maxResults_3_5e117e,
			);
			$content .="
			<div class=\"lastNews\">
			<div class=\"newsTop\">
			...
			";
			//FUNCTION TAG START 6_01fef8
			$parameters_6_01fef8 = array ('selected' => CMS_polymod_definition_parsing::replaceVars("3", $replace),);
			$object_6_01fef8 = &$object[1];
			if (method_exists($object_6_01fef8, "rss")) {
				$content .= CMS_polymod_definition_parsing::replaceVars($object_6_01fef8->rss($parameters_6_01fef8, array (
					0 =>
					array (
						'textnode' => '
						',
				...
				CMS_grandFather::raiseError("Malformed atm-function tag : can't found method rss on object : ".get_class($object_6_01fef8));
			}
			//FUNCTION TAG END 6_01fef8
			$content .="
			</div>
			</div>
			</div>
			...
			$content_3_5e117e.= CMS_polymod_definition_parsing::replaceVars($content, $replace);
		}
		$content = $content_3_5e117e; //retrieve previous content var if any
		$replace = $replace_3_5e117e; //retrieve previous replace vars if any
		$object[$objectDefinition_lastNews] = $object_3_5e117e; //retrieve previous object search if any
	}
	//RESULT lastNews TAG END 3_5e117e
	//destroy search and results lastNews objects
	unset($search_lastNews);
	unset($results_lastNews);
	//SEARCH lastNews TAG END 2_96f644
