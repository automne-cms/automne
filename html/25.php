...
if (!isset($cms_page_included) && !$_POST && !$_GET) {
	...
	header('Location: http://127.0.0.1/web/demo/25-modeles.php');
	exit;
}
...
<a href="http://127.0.0.1/web/demo/24-documentation.php">Fonctionnalités</a>
 ...
 &gt; 
	...
	<div class="imgRight">
		<?php //Generated by : $Id: 25.php,v 1.4 2009/10/28 16:31:34 sebastien Exp $
$content = "";
$replace = "";
...
//SEARCH mediaresult TAG START 2_41f3b3
$objectDefinition_mediaresult = '2';
if (!isset($objectDefinitions[$objectDefinition_mediaresult])) {
	$objectDefinitions[$objectDefinition_mediaresult] = new CMS_poly_object_definition($objectDefinition_mediaresult);
}
//public search ?
$public_2_41f3b3 = isset($public_search) ? $public_search : false;
//get search params
$search_mediaresult = new CMS_object_search($objectDefinitions[$objectDefinition_mediaresult], $public_2_41f3b3);
$launchSearch_mediaresult = true;
//add search conditions if any
if (isset($blockAttributes['search']['mediaresult']['item'])) {
	$values_3_e919a5 = array (
		'search' => 'mediaresult',
		'type' => 'item',
		'value' => 'block',
		'mandatory' => 'true',
	);
	$values_3_e919a5['value'] = $blockAttributes['search']['mediaresult']['item'];
	if ($values_3_e919a5['type'] == 'publication date after' || $values_3_e919a5['type'] == 'publication date before') {
		//convert DB format to current language format
		$dt = new CMS_date();
		$dt->setFromDBValue($values_3_e919a5['value']);
		$values_3_e919a5['value'] = $dt->getLocalizedDate($cms_language->getDateFormat());
	}
	$launchSearch_mediaresult = (CMS_polymod_definition_parsing::addSearchCondition($search_mediaresult, $values_3_e919a5)) ? $launchSearch_mediaresult : false;
} elseif (true == true) {
	//search parameter is mandatory and no value found
	$launchSearch_mediaresult = false;
}
//RESULT mediaresult TAG START 4_740911
...
if ($results_mediaresult) {
	$object_4_740911 = (isset($object[$objectDefinition_mediaresult])) ? $object[$objectDefinition_mediaresult] : ""; //save previous object search if any
	$replace_4_740911 = $replace; //save previous replace vars if any
	$count_4_740911 = 0;
	$content_4_740911 = $content; //save previous content var if any
	$maxPages_4_740911 = $search_mediaresult->getMaxPages();
	$maxResults_4_740911 = $search_mediaresult->getNumRows();
	foreach ($results_mediaresult as $object[$objectDefinition_mediaresult]) {
		$content = "";
		$replace["atm-search"] = array (
			"{resultid}" 	=> (isset($resultID_mediaresult)) ? $resultID_mediaresult : $object[$objectDefinition_mediaresult]->getID(),
			"{firstresult}" => (!$count_4_740911) ? 1 : 0,
			"{lastresult}" 	=> ($count_4_740911 == sizeof($results_mediaresult)-1) ? 1 : 0,
			"{resultcount}" => ($count_4_740911+1),
			"{maxpages}"    => $maxPages_4_740911,
			"{currentpage}" => ($search_mediaresult->getAttribute('page')+1),
			"{maxresults}"  => $maxResults_4_740911,
		);
		//IF TAG START 5_c94f8d
		...
		//IF TAG START 8_fdfebb
		$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('fileExtension',''))." == 'flv'", $replace);
		if ($ifcondition) {
			$func = create_function("","return (".$ifcondition.");");
			if ($func()) {
				//IF TAG START 9_bc10d2
				$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
				if ($ifcondition) {
					$func = create_function("","return (".$ifcondition.");");
					if ($func()) {
		...
		if ($ifcondition) {
			...
			if ($func()) {
				...
				}//IF TAG END 12_751ff5
			}
		}//IF TAG END 11_7794bd
		//IF TAG START 13_3d5bce
		...
		if ($ifcondition) {
			...
			if ($func()) {
				...
				}//IF TAG END 14_828f54
				//IF TAG START 15_872895
				$ifcondition = CMS_polymod_definition_parsing::replaceVars("!".CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
				if ($ifcondition) {
					$func = create_function("","return (".$ifcondition.");");
					if ($func()) {
		...
		}//IF TAG END 13_3d5bce
		$count_4_740911++;
		//do all result vars replacement
		$content_4_740911.= CMS_polymod_definition_parsing::replaceVars($content, $replace);
	}
	$content = $content_4_740911; //retrieve previous content var if any
	$replace = $replace_4_740911; //retrieve previous replace vars if any
	$object[$objectDefinition_mediaresult] = $object_4_740911; //retrieve previous object search if any
}
//RESULT mediaresult TAG END 4_740911
//destroy search and results mediaresult objects
unset($search_mediaresult);
unset($results_mediaresult);
//SEARCH mediaresult TAG END 2_41f3b3
