...
if (!isset($cms_page_included) && !$_POST && !$_GET) {
	...
	header('Location: http://127.0.0.1/web/demo/28-administration.php');
	exit;
}
...
<a href="http://127.0.0.1/web/demo/24-documentation.php">Fonctionnalités</a>
 ...
 &gt; 
	...
	<div class="imgRight">
		<?php //Generated by : $Id: 28.php,v 1.4 2009/10/28 16:31:34 sebastien Exp $
$content = "";
$replace = "";
...
//SEARCH mediaresult TAG START 2_557b63
$objectDefinition_mediaresult = '2';
if (!isset($objectDefinitions[$objectDefinition_mediaresult])) {
	$objectDefinitions[$objectDefinition_mediaresult] = new CMS_poly_object_definition($objectDefinition_mediaresult);
}
//public search ?
$public_2_557b63 = isset($public_search) ? $public_search : false;
//get search params
$search_mediaresult = new CMS_object_search($objectDefinitions[$objectDefinition_mediaresult], $public_2_557b63);
$launchSearch_mediaresult = true;
//add search conditions if any
if (isset($blockAttributes['search']['mediaresult']['item'])) {
	$values_3_3acbd8 = array (
		'search' => 'mediaresult',
		'type' => 'item',
		'value' => 'block',
		'mandatory' => 'true',
	);
	$values_3_3acbd8['value'] = $blockAttributes['search']['mediaresult']['item'];
	if ($values_3_3acbd8['type'] == 'publication date after' || $values_3_3acbd8['type'] == 'publication date before') {
		//convert DB format to current language format
		$dt = new CMS_date();
		$dt->setFromDBValue($values_3_3acbd8['value']);
		$values_3_3acbd8['value'] = $dt->getLocalizedDate($cms_language->getDateFormat());
	}
	$launchSearch_mediaresult = (CMS_polymod_definition_parsing::addSearchCondition($search_mediaresult, $values_3_3acbd8)) ? $launchSearch_mediaresult : false;
} elseif (true == true) {
	//search parameter is mandatory and no value found
	$launchSearch_mediaresult = false;
}
//RESULT mediaresult TAG START 4_fa8b7d
...
if ($results_mediaresult) {
	$object_4_fa8b7d = (isset($object[$objectDefinition_mediaresult])) ? $object[$objectDefinition_mediaresult] : ""; //save previous object search if any
	$replace_4_fa8b7d = $replace; //save previous replace vars if any
	$count_4_fa8b7d = 0;
	$content_4_fa8b7d = $content; //save previous content var if any
	$maxPages_4_fa8b7d = $search_mediaresult->getMaxPages();
	$maxResults_4_fa8b7d = $search_mediaresult->getNumRows();
	foreach ($results_mediaresult as $object[$objectDefinition_mediaresult]) {
		$content = "";
		$replace["atm-search"] = array (
			"{resultid}" 	=> (isset($resultID_mediaresult)) ? $resultID_mediaresult : $object[$objectDefinition_mediaresult]->getID(),
			"{firstresult}" => (!$count_4_fa8b7d) ? 1 : 0,
			"{lastresult}" 	=> ($count_4_fa8b7d == sizeof($results_mediaresult)-1) ? 1 : 0,
			"{resultcount}" => ($count_4_fa8b7d+1),
			"{maxpages}"    => $maxPages_4_fa8b7d,
			"{currentpage}" => ($search_mediaresult->getAttribute('page')+1),
			"{maxresults}"  => $maxResults_4_fa8b7d,
		);
		//IF TAG START 5_2913cb
		...
		//IF TAG START 8_005275
		$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('fileExtension',''))." == 'flv'", $replace);
		if ($ifcondition) {
			$func = create_function("","return (".$ifcondition.");");
			if ($func()) {
				//IF TAG START 9_92294b
				$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
				if ($ifcondition) {
					$func = create_function("","return (".$ifcondition.");");
					if ($func()) {
		...
		if ($ifcondition) {
			...
			if ($func()) {
				...
				}//IF TAG END 12_575def
			}
		}//IF TAG END 11_a26a7d
		//IF TAG START 13_0f164f
		...
		if ($ifcondition) {
			...
			if ($func()) {
				...
				}//IF TAG END 14_c7b44d
				//IF TAG START 15_403b7e
				$ifcondition = CMS_polymod_definition_parsing::replaceVars("!".CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
				if ($ifcondition) {
					$func = create_function("","return (".$ifcondition.");");
					if ($func()) {
		...
		}//IF TAG END 13_0f164f
		$count_4_fa8b7d++;
		//do all result vars replacement
		$content_4_fa8b7d.= CMS_polymod_definition_parsing::replaceVars($content, $replace);
	}
	$content = $content_4_fa8b7d; //retrieve previous content var if any
	$replace = $replace_4_fa8b7d; //retrieve previous replace vars if any
	$object[$objectDefinition_mediaresult] = $object_4_fa8b7d; //retrieve previous object search if any
}
//RESULT mediaresult TAG END 4_fa8b7d
//destroy search and results mediaresult objects
unset($search_mediaresult);
unset($results_mediaresult);
//SEARCH mediaresult TAG END 2_557b63
