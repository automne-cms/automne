...
<a href="http://127.0.0.1/web/fr/24-documentation.php">Fonctionnalités</a>
 ...
 &gt; 
	...
	<div class="imgRight">
		<?php //Generated by : $Id: 37.php,v 1.2 2009/06/23 16:00:41 sebastien Exp $
$content = "";
$replace = "";
...
//SEARCH mediaresult TAG START 2_aee303
$objectDefinition_mediaresult = '2';
if (!isset($objectDefinitions[$objectDefinition_mediaresult])) {
	$objectDefinitions[$objectDefinition_mediaresult] = new CMS_poly_object_definition($objectDefinition_mediaresult);
}
//public search ?
$public_2_aee303 = isset($public_search) ? $public_search : false;
//get search params
$search_mediaresult = new CMS_object_search($objectDefinitions[$objectDefinition_mediaresult], $public_2_aee303);
$launchSearch_mediaresult = true;
//add search conditions if any
if (isset($blockAttributes['search']['mediaresult']['item'])) {
	$values_3_12299e = array (
		'search' => 'mediaresult',
		'type' => 'item',
		'value' => 'block',
		'mandatory' => 'true',
	);
	$values_3_12299e['value'] = $blockAttributes['search']['mediaresult']['item'];
	if ($values_3_12299e['type'] == 'publication date after' || $values_3_12299e['type'] == 'publication date before') {
		//convert DB format to current language format
		$dt = new CMS_date();
		$dt->setFromDBValue($values_3_12299e['value']);
		$values_3_12299e['value'] = $dt->getLocalizedDate($cms_language->getDateFormat());
	}
	$launchSearch_mediaresult = (CMS_polymod_definition_parsing::addSearchCondition($search_mediaresult, $values_3_12299e)) ? $launchSearch_mediaresult : false;
} elseif (true == true) {
	//search parameter is mandatory and no value found
	$launchSearch_mediaresult = false;
}
//RESULT mediaresult TAG START 4_a6ce34
...
if ($results_mediaresult) {
	$object_4_a6ce34 = $object[$objectDefinition_mediaresult]; //save previous object search if any
	$replace_4_a6ce34 = $replace; //save previous replace vars if any
	$count_4_a6ce34 = 0;
	$content_4_a6ce34 = $content; //save previous content var if any
	$maxPages_4_a6ce34 = $search_mediaresult->getMaxPages();
	$maxResults_4_a6ce34 = $search_mediaresult->getNumRows();
	foreach ($results_mediaresult as $object[$objectDefinition_mediaresult]) {
		$content = "";
		$replace["atm-search"] = array (
			"{resultid}" 	=> (isset($resultID_mediaresult)) ? $resultID_mediaresult : $object[$objectDefinition_mediaresult]->getID(),
			"{firstresult}" => (!$count_4_a6ce34) ? 1 : 0,
			"{lastresult}" 	=> ($count_4_a6ce34 == sizeof($results_mediaresult)-1) ? 1 : 0,
			"{resultcount}" => ($count_4_a6ce34+1),
			"{maxpages}"    => $maxPages_4_a6ce34,
			"{currentpage}" => ($search_mediaresult->getAttribute('page')+1),
			"{maxresults}"  => $maxResults_4_a6ce34,
		);
		//IF TAG START 5_3f2132
		...
		//IF TAG START 8_82d1f0
		$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('fileExtension',''))." == 'flv'", $replace);
		if ($ifcondition) {
			$func = create_function("","return (".$ifcondition.");");
			if ($func()) {
				//IF TAG START 9_0ae1de
				$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
				if ($ifcondition) {
					$func = create_function("","return (".$ifcondition.");");
					if ($func()) {
		...
		if ($ifcondition) {
			...
			if ($func()) {
				...
				}//IF TAG END 12_788bf6
			}
		}//IF TAG END 11_4bf74c
		//IF TAG START 13_f32cdd
		...
		if ($ifcondition) {
			...
			if ($func()) {
				...
				}//IF TAG END 14_eb4bea
				//IF TAG START 15_c9ecc8
				$ifcondition = CMS_polymod_definition_parsing::replaceVars("!".CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
				if ($ifcondition) {
					$func = create_function("","return (".$ifcondition.");");
					if ($func()) {
		...
		}//IF TAG END 13_f32cdd
		$count_4_a6ce34++;
		//do all result vars replacement
		$content_4_a6ce34.= CMS_polymod_definition_parsing::replaceVars($content, $replace);
	}
	$content = $content_4_a6ce34; //retrieve previous content var if any
	$replace = $replace_4_a6ce34; //retrieve previous replace vars if any
	$object[$objectDefinition_mediaresult] = $object_4_a6ce34; //retrieve previous object search if any
}
//RESULT mediaresult TAG END 4_a6ce34
//destroy search and results mediaresult objects
unset($search_mediaresult);
unset($results_mediaresult);
//SEARCH mediaresult TAG END 2_aee303
