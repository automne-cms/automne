...
<a href="http://127.0.0.1/web/demo/24-documentation.php">Fonctionnalités</a>
 ...
 &gt; 
	...
	<div class="imgRight">
		<?php //Generated by : $Id: 37.php,v 1.4 2009/10/28 16:31:34 sebastien Exp $
$content = "";
$replace = "";
...
//SEARCH mediaresult TAG START 2_39e35d
$objectDefinition_mediaresult = '2';
if (!isset($objectDefinitions[$objectDefinition_mediaresult])) {
	$objectDefinitions[$objectDefinition_mediaresult] = new CMS_poly_object_definition($objectDefinition_mediaresult);
}
//public search ?
$public_2_39e35d = isset($public_search) ? $public_search : false;
//get search params
$search_mediaresult = new CMS_object_search($objectDefinitions[$objectDefinition_mediaresult], $public_2_39e35d);
$launchSearch_mediaresult = true;
//add search conditions if any
if (isset($blockAttributes['search']['mediaresult']['item'])) {
	$values_3_0c4b5e = array (
		'search' => 'mediaresult',
		'type' => 'item',
		'value' => 'block',
		'mandatory' => 'true',
	);
	$values_3_0c4b5e['value'] = $blockAttributes['search']['mediaresult']['item'];
	if ($values_3_0c4b5e['type'] == 'publication date after' || $values_3_0c4b5e['type'] == 'publication date before') {
		//convert DB format to current language format
		$dt = new CMS_date();
		$dt->setFromDBValue($values_3_0c4b5e['value']);
		$values_3_0c4b5e['value'] = $dt->getLocalizedDate($cms_language->getDateFormat());
	}
	$launchSearch_mediaresult = (CMS_polymod_definition_parsing::addSearchCondition($search_mediaresult, $values_3_0c4b5e)) ? $launchSearch_mediaresult : false;
} elseif (true == true) {
	//search parameter is mandatory and no value found
	$launchSearch_mediaresult = false;
}
//RESULT mediaresult TAG START 4_413546
...
if ($results_mediaresult) {
	$object_4_413546 = (isset($object[$objectDefinition_mediaresult])) ? $object[$objectDefinition_mediaresult] : ""; //save previous object search if any
	$replace_4_413546 = $replace; //save previous replace vars if any
	$count_4_413546 = 0;
	$content_4_413546 = $content; //save previous content var if any
	$maxPages_4_413546 = $search_mediaresult->getMaxPages();
	$maxResults_4_413546 = $search_mediaresult->getNumRows();
	foreach ($results_mediaresult as $object[$objectDefinition_mediaresult]) {
		$content = "";
		$replace["atm-search"] = array (
			"{resultid}" 	=> (isset($resultID_mediaresult)) ? $resultID_mediaresult : $object[$objectDefinition_mediaresult]->getID(),
			"{firstresult}" => (!$count_4_413546) ? 1 : 0,
			"{lastresult}" 	=> ($count_4_413546 == sizeof($results_mediaresult)-1) ? 1 : 0,
			"{resultcount}" => ($count_4_413546+1),
			"{maxpages}"    => $maxPages_4_413546,
			"{currentpage}" => ($search_mediaresult->getAttribute('page')+1),
			"{maxresults}"  => $maxResults_4_413546,
		);
		//IF TAG START 5_78b5b5
		...
		//IF TAG START 8_777791
		$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('fileExtension',''))." == 'flv'", $replace);
		if ($ifcondition) {
			$func = create_function("","return (".$ifcondition.");");
			if ($func()) {
				//IF TAG START 9_21d506
				$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
				if ($ifcondition) {
					$func = create_function("","return (".$ifcondition.");");
					if ($func()) {
		...
		if ($ifcondition) {
			...
			if ($func()) {
				...
				}//IF TAG END 12_99076e
			}
		}//IF TAG END 11_d5d257
		//IF TAG START 13_666a80
		...
		if ($ifcondition) {
			...
			if ($func()) {
				...
				}//IF TAG END 14_e8b261
				//IF TAG START 15_4d3ede
				$ifcondition = CMS_polymod_definition_parsing::replaceVars("!".CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
				if ($ifcondition) {
					$func = create_function("","return (".$ifcondition.");");
					if ($func()) {
		...
		}//IF TAG END 13_666a80
		$count_4_413546++;
		//do all result vars replacement
		$content_4_413546.= CMS_polymod_definition_parsing::replaceVars($content, $replace);
	}
	$content = $content_4_413546; //retrieve previous content var if any
	$replace = $replace_4_413546; //retrieve previous replace vars if any
	$object[$objectDefinition_mediaresult] = $object_4_413546; //retrieve previous object search if any
}
//RESULT mediaresult TAG END 4_413546
//destroy search and results mediaresult objects
unset($search_mediaresult);
unset($results_mediaresult);
//SEARCH mediaresult TAG END 2_39e35d
