...
if(!APPLICATION_ENFORCES_ACCESS_CONTROL || (isset($cms_user) && is_a($cms_user, 'CMS_profile_user') && $cms_user->hasModuleClearance('pmedia', CLEARANCE_MODULE_VIEW))){
	...
	//SEARCH mediaresult TAG START 2_8a630a
	$objectDefinition_mediaresult = '2';
	if (!isset($objectDefinitions[$objectDefinition_mediaresult])) {
		$objectDefinitions[$objectDefinition_mediaresult] = new CMS_poly_object_definition($objectDefinition_mediaresult);
	}
	//public search ?
	$public_2_8a630a = isset($public_search) ? $public_search : false;
	//get search params
	$search_mediaresult = new CMS_object_search($objectDefinitions[$objectDefinition_mediaresult], $public_2_8a630a);
	$launchSearch_mediaresult = true;
	//add search conditions if any
	if (isset($blockAttributes['search']['mediaresult']['item'])) {
		$values_3_a8b0d1 = array (
			'search' => 'mediaresult',
			'type' => 'item',
			'value' => 'block',
			'mandatory' => 'true',
		);
		$values_3_a8b0d1['value'] = $blockAttributes['search']['mediaresult']['item'];
		if ($values_3_a8b0d1['type'] == 'publication date after' || $values_3_a8b0d1['type'] == 'publication date before') {
			//convert DB format to current language format
			$dt = new CMS_date();
			$dt->setFromDBValue($values_3_a8b0d1['value']);
			$values_3_a8b0d1['value'] = $dt->getLocalizedDate($cms_language->getDateFormat());
		}
		$launchSearch_mediaresult = (CMS_polymod_definition_parsing::addSearchCondition($search_mediaresult, $values_3_a8b0d1)) ? $launchSearch_mediaresult : false;
	} elseif (true == true) {
		//search parameter is mandatory and no value found
		$launchSearch_mediaresult = false;
	}
	//RESULT mediaresult TAG START 4_53a2a7
	...
	if ($results_mediaresult) {
		$object_4_53a2a7 = (isset($object[$objectDefinition_mediaresult])) ? $object[$objectDefinition_mediaresult] : ""; //save previous object search if any
		$replace_4_53a2a7 = $replace; //save previous replace vars if any
		$count_4_53a2a7 = 0;
		$content_4_53a2a7 = $content; //save previous content var if any
		$maxPages_4_53a2a7 = $search_mediaresult->getMaxPages();
		$maxResults_4_53a2a7 = $search_mediaresult->getNumRows();
		foreach ($results_mediaresult as $object[$objectDefinition_mediaresult]) {
			$content = "";
			$replace["atm-search"] = array (
				"{resultid}" 	=> (isset($resultID_mediaresult)) ? $resultID_mediaresult : $object[$objectDefinition_mediaresult]->getID(),
				"{firstresult}" => (!$count_4_53a2a7) ? 1 : 0,
				"{lastresult}" 	=> ($count_4_53a2a7 == sizeof($results_mediaresult)-1) ? 1 : 0,
				"{resultcount}" => ($count_4_53a2a7+1),
				"{maxpages}"    => $maxPages_4_53a2a7,
				"{currentpage}" => ($search_mediaresult->getAttribute('page')+1),
				"{maxresults}"  => $maxResults_4_53a2a7,
			);
			//IF TAG START 5_f696a4
			...
			//IF TAG START 8_3aa5a9
			$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('fileExtension',''))." == 'flv'", $replace);
			if ($ifcondition) {
				$func = create_function("","return (".$ifcondition.");");
				if ($func()) {
					//IF TAG START 9_0fc007
					$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
					if ($ifcondition) {
						$func = create_function("","return (".$ifcondition.");");
						if ($func()) {
			...
			if ($ifcondition) {
				...
				if ($func()) {
					...
					}//IF TAG END 12_7cfa9a
				}
			}//IF TAG END 11_5ca385
			//IF TAG START 13_1ab50d
			...
			if ($ifcondition) {
				...
				if ($func()) {
					...
					}//IF TAG END 14_feedb9
					//IF TAG START 15_ecbc5d
					$ifcondition = CMS_polymod_definition_parsing::replaceVars("!".CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
					if ($ifcondition) {
						$func = create_function("","return (".$ifcondition.");");
						if ($func()) {
					...
					}//IF TAG END 15_ecbc5d
					$content .="
					</div>
					";
				}
			}//IF TAG END 13_1ab50d
			$count_4_53a2a7++;
			//do all result vars replacement
			$content_4_53a2a7.= CMS_polymod_definition_parsing::replaceVars($content, $replace);
		}
		$content = $content_4_53a2a7; //retrieve previous content var if any
		$replace = $replace_4_53a2a7; //retrieve previous replace vars if any
		$object[$objectDefinition_mediaresult] = $object_4_53a2a7; //retrieve previous object search if any
	}
	//RESULT mediaresult TAG END 4_53a2a7
	//destroy search and results mediaresult objects
	unset($search_mediaresult);
	unset($results_mediaresult);
	//SEARCH mediaresult TAG END 2_8a630a
