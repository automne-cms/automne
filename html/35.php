<?php //Generated on Tue, 09 Mar 2010 12:18:31 +0100 by Automne (TM) 4.0.1
...
if(!APPLICATION_ENFORCES_ACCESS_CONTROL || (isset($cms_user) && is_a($cms_user, 'CMS_profile_user') && $cms_user->hasModuleClearance('pmedia', CLEARANCE_MODULE_VIEW))){
	...
	//SEARCH mediaresult TAG START 2_1a30de
	$objectDefinition_mediaresult = '2';
	if (!isset($objectDefinitions[$objectDefinition_mediaresult])) {
		$objectDefinitions[$objectDefinition_mediaresult] = new CMS_poly_object_definition($objectDefinition_mediaresult);
	}
	//public search ?
	$public_2_1a30de = isset($public_search) ? $public_search : false;
	//get search params
	$search_mediaresult = new CMS_object_search($objectDefinitions[$objectDefinition_mediaresult], $public_2_1a30de);
	$launchSearch_mediaresult = true;
	//add search conditions if any
	if (isset($blockAttributes['search']['mediaresult']['item'])) {
		$values_3_d7d27a = array (
			'search' => 'mediaresult',
			'type' => 'item',
			'value' => 'block',
			'mandatory' => 'true',
		);
		$values_3_d7d27a['value'] = $blockAttributes['search']['mediaresult']['item'];
		if ($values_3_d7d27a['type'] == 'publication date after' || $values_3_d7d27a['type'] == 'publication date before') {
			//convert DB format to current language format
			$dt = new CMS_date();
			$dt->setFromDBValue($values_3_d7d27a['value']);
			$values_3_d7d27a['value'] = $dt->getLocalizedDate($cms_language->getDateFormat());
		}
		$launchSearch_mediaresult = (CMS_polymod_definition_parsing::addSearchCondition($search_mediaresult, $values_3_d7d27a)) ? $launchSearch_mediaresult : false;
	} elseif (true == true) {
		//search parameter is mandatory and no value found
		$launchSearch_mediaresult = false;
	}
	//RESULT mediaresult TAG START 4_c8df6d
	...
	if ($results_mediaresult) {
		$object_4_c8df6d = (isset($object[$objectDefinition_mediaresult])) ? $object[$objectDefinition_mediaresult] : ""; //save previous object search if any
		$replace_4_c8df6d = $replace; //save previous replace vars if any
		$count_4_c8df6d = 0;
		$content_4_c8df6d = $content; //save previous content var if any
		$maxPages_4_c8df6d = $search_mediaresult->getMaxPages();
		$maxResults_4_c8df6d = $search_mediaresult->getNumRows();
		foreach ($results_mediaresult as $object[$objectDefinition_mediaresult]) {
			$content = "";
			$replace["atm-search"] = array (
				"{resultid}" 	=> (isset($resultID_mediaresult)) ? $resultID_mediaresult : $object[$objectDefinition_mediaresult]->getID(),
				"{firstresult}" => (!$count_4_c8df6d) ? 1 : 0,
				"{lastresult}" 	=> ($count_4_c8df6d == sizeof($results_mediaresult)-1) ? 1 : 0,
				"{resultcount}" => ($count_4_c8df6d+1),
				"{maxpages}"    => $maxPages_4_c8df6d,
				"{currentpage}" => ($search_mediaresult->getAttribute('page')+1),
				"{maxresults}"  => $maxResults_4_c8df6d,
			);
			//IF TAG START 5_1c4e3e
			...
			//IF TAG START 8_3e1c92
			$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('fileExtension',''))." == 'flv'", $replace);
			if ($ifcondition) {
				$func = create_function("","return (".$ifcondition.");");
				if ($func()) {
					//IF TAG START 9_0be420
					$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
					if ($ifcondition) {
						$func = create_function("","return (".$ifcondition.");");
						if ($func()) {
			...
			if ($ifcondition) {
				...
				if ($func()) {
					...
					}//IF TAG END 12_650895
				}
			}//IF TAG END 11_f67020
			//IF TAG START 13_8349bf
			...
			if ($ifcondition) {
				...
				if ($func()) {
					...
					}//IF TAG END 14_d27842
					//IF TAG START 15_d29f19
					$ifcondition = CMS_polymod_definition_parsing::replaceVars("!".CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
					if ($ifcondition) {
						$func = create_function("","return (".$ifcondition.");");
						if ($func()) {
					...
					}//IF TAG END 15_d29f19
					$content .="
					</div>
					";
				}
			}//IF TAG END 13_8349bf
			$count_4_c8df6d++;
			//do all result vars replacement
			$content_4_c8df6d.= CMS_polymod_definition_parsing::replaceVars($content, $replace);
		}
		$content = $content_4_c8df6d; //retrieve previous content var if any
		$replace = $replace_4_c8df6d; //retrieve previous replace vars if any
		$object[$objectDefinition_mediaresult] = $object_4_c8df6d; //retrieve previous object search if any
	}
	//RESULT mediaresult TAG END 4_c8df6d
	//destroy search and results mediaresult objects
	unset($search_mediaresult);
	unset($results_mediaresult);
	//SEARCH mediaresult TAG END 2_1a30de
