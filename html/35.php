...
<h2>Principe de gestion des droits</h2>
	...
	<div class="imgRight">
		<?php //Generated by : $Id: 35.php,v 1.2 2009/06/23 16:00:41 sebastien Exp $
$content = "";
$replace = "";
...
//SEARCH mediaresult TAG START 2_2ee4fc
$objectDefinition_mediaresult = '2';
if (!isset($objectDefinitions[$objectDefinition_mediaresult])) {
	$objectDefinitions[$objectDefinition_mediaresult] = new CMS_poly_object_definition($objectDefinition_mediaresult);
}
//public search ?
$public_2_2ee4fc = isset($public_search) ? $public_search : false;
//get search params
$search_mediaresult = new CMS_object_search($objectDefinitions[$objectDefinition_mediaresult], $public_2_2ee4fc);
$launchSearch_mediaresult = true;
//add search conditions if any
if (isset($blockAttributes['search']['mediaresult']['item'])) {
	$values_3_296ef4 = array (
		'search' => 'mediaresult',
		'type' => 'item',
		'value' => 'block',
		'mandatory' => 'true',
	);
	$values_3_296ef4['value'] = $blockAttributes['search']['mediaresult']['item'];
	if ($values_3_296ef4['type'] == 'publication date after' || $values_3_296ef4['type'] == 'publication date before') {
		//convert DB format to current language format
		$dt = new CMS_date();
		$dt->setFromDBValue($values_3_296ef4['value']);
		$values_3_296ef4['value'] = $dt->getLocalizedDate($cms_language->getDateFormat());
	}
	$launchSearch_mediaresult = (CMS_polymod_definition_parsing::addSearchCondition($search_mediaresult, $values_3_296ef4)) ? $launchSearch_mediaresult : false;
} elseif (true == true) {
	//search parameter is mandatory and no value found
	$launchSearch_mediaresult = false;
}
//RESULT mediaresult TAG START 4_e89f31
...
if ($results_mediaresult) {
	$object_4_e89f31 = $object[$objectDefinition_mediaresult]; //save previous object search if any
	$replace_4_e89f31 = $replace; //save previous replace vars if any
	$count_4_e89f31 = 0;
	$content_4_e89f31 = $content; //save previous content var if any
	$maxPages_4_e89f31 = $search_mediaresult->getMaxPages();
	$maxResults_4_e89f31 = $search_mediaresult->getNumRows();
	foreach ($results_mediaresult as $object[$objectDefinition_mediaresult]) {
		$content = "";
		$replace["atm-search"] = array (
			"{resultid}" 	=> (isset($resultID_mediaresult)) ? $resultID_mediaresult : $object[$objectDefinition_mediaresult]->getID(),
			"{firstresult}" => (!$count_4_e89f31) ? 1 : 0,
			"{lastresult}" 	=> ($count_4_e89f31 == sizeof($results_mediaresult)-1) ? 1 : 0,
			"{resultcount}" => ($count_4_e89f31+1),
			"{maxpages}"    => $maxPages_4_e89f31,
			"{currentpage}" => ($search_mediaresult->getAttribute('page')+1),
			"{maxresults}"  => $maxResults_4_e89f31,
		);
		//IF TAG START 5_c2034d
		...
		//IF TAG START 8_2a2fea
		$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('fileExtension',''))." == 'flv'", $replace);
		if ($ifcondition) {
			$func = create_function("","return (".$ifcondition.");");
			if ($func()) {
				//IF TAG START 9_2235ae
				$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
				if ($ifcondition) {
					$func = create_function("","return (".$ifcondition.");");
					if ($func()) {
		...
		if ($ifcondition) {
			...
			if ($func()) {
				...
				}//IF TAG END 12_82f79b
			}
		}//IF TAG END 11_765f7b
		//IF TAG START 13_1f31a5
		...
		if ($ifcondition) {
			...
			if ($func()) {
				...
				}//IF TAG END 14_b155f7
				//IF TAG START 15_65fd6d
				$ifcondition = CMS_polymod_definition_parsing::replaceVars("!".CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
				if ($ifcondition) {
					$func = create_function("","return (".$ifcondition.");");
					if ($func()) {
		...
		}//IF TAG END 13_1f31a5
		$count_4_e89f31++;
		//do all result vars replacement
		$content_4_e89f31.= CMS_polymod_definition_parsing::replaceVars($content, $replace);
	}
	$content = $content_4_e89f31; //retrieve previous content var if any
	$replace = $replace_4_e89f31; //retrieve previous replace vars if any
	$object[$objectDefinition_mediaresult] = $object_4_e89f31; //retrieve previous object search if any
}
//RESULT mediaresult TAG END 4_e89f31
//destroy search and results mediaresult objects
unset($search_mediaresult);
unset($results_mediaresult);
//SEARCH mediaresult TAG END 2_2ee4fc
