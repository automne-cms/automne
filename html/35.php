...
<h2>Principe de gestion des droits</h2>
	...
	<div class="imgRight">
		<?php //Generated by : $Id: 35.php,v 1.5 2009/11/17 16:04:28 sebastien Exp $
if(!APPLICATION_ENFORCES_ACCESS_CONTROL || (isset($cms_user) && is_a($cms_user, 'CMS_profile_user') && $cms_user->hasModuleClearance('pmedia', CLEARANCE_MODULE_VIEW))){
	...
	//SEARCH mediaresult TAG START 2_3b5962
	$objectDefinition_mediaresult = '2';
	if (!isset($objectDefinitions[$objectDefinition_mediaresult])) {
		$objectDefinitions[$objectDefinition_mediaresult] = new CMS_poly_object_definition($objectDefinition_mediaresult);
	}
	//public search ?
	$public_2_3b5962 = isset($public_search) ? $public_search : false;
	//get search params
	$search_mediaresult = new CMS_object_search($objectDefinitions[$objectDefinition_mediaresult], $public_2_3b5962);
	$launchSearch_mediaresult = true;
	//add search conditions if any
	if (isset($blockAttributes['search']['mediaresult']['item'])) {
		$values_3_79413e = array (
			'search' => 'mediaresult',
			'type' => 'item',
			'value' => 'block',
			'mandatory' => 'true',
		);
		$values_3_79413e['value'] = $blockAttributes['search']['mediaresult']['item'];
		if ($values_3_79413e['type'] == 'publication date after' || $values_3_79413e['type'] == 'publication date before') {
			//convert DB format to current language format
			$dt = new CMS_date();
			$dt->setFromDBValue($values_3_79413e['value']);
			$values_3_79413e['value'] = $dt->getLocalizedDate($cms_language->getDateFormat());
		}
		$launchSearch_mediaresult = (CMS_polymod_definition_parsing::addSearchCondition($search_mediaresult, $values_3_79413e)) ? $launchSearch_mediaresult : false;
	} elseif (true == true) {
		//search parameter is mandatory and no value found
		$launchSearch_mediaresult = false;
	}
	//RESULT mediaresult TAG START 4_e7e5bd
	...
	if ($results_mediaresult) {
		$object_4_e7e5bd = (isset($object[$objectDefinition_mediaresult])) ? $object[$objectDefinition_mediaresult] : ""; //save previous object search if any
		$replace_4_e7e5bd = $replace; //save previous replace vars if any
		$count_4_e7e5bd = 0;
		$content_4_e7e5bd = $content; //save previous content var if any
		$maxPages_4_e7e5bd = $search_mediaresult->getMaxPages();
		$maxResults_4_e7e5bd = $search_mediaresult->getNumRows();
		foreach ($results_mediaresult as $object[$objectDefinition_mediaresult]) {
			$content = "";
			$replace["atm-search"] = array (
				"{resultid}" 	=> (isset($resultID_mediaresult)) ? $resultID_mediaresult : $object[$objectDefinition_mediaresult]->getID(),
				"{firstresult}" => (!$count_4_e7e5bd) ? 1 : 0,
				"{lastresult}" 	=> ($count_4_e7e5bd == sizeof($results_mediaresult)-1) ? 1 : 0,
				"{resultcount}" => ($count_4_e7e5bd+1),
				"{maxpages}"    => $maxPages_4_e7e5bd,
				"{currentpage}" => ($search_mediaresult->getAttribute('page')+1),
				"{maxresults}"  => $maxResults_4_e7e5bd,
			);
			//IF TAG START 5_70fc0b
			...
			//IF TAG START 8_7d8b12
			$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('fileExtension',''))." == 'flv'", $replace);
			if ($ifcondition) {
				$func = create_function("","return (".$ifcondition.");");
				if ($func()) {
					//IF TAG START 9_42bca9
					$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
					if ($ifcondition) {
						$func = create_function("","return (".$ifcondition.");");
						if ($func()) {
			...
			if ($ifcondition) {
				...
				if ($func()) {
					...
					}//IF TAG END 12_1d5ec1
				}
			}//IF TAG END 11_6c66d9
			//IF TAG START 13_c4b784
			...
			if ($ifcondition) {
				...
				if ($func()) {
					...
					}//IF TAG END 14_f099df
					//IF TAG START 15_cbbf13
					$ifcondition = CMS_polymod_definition_parsing::replaceVars("!".CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
					if ($ifcondition) {
						$func = create_function("","return (".$ifcondition.");");
						if ($func()) {
					...
					}//IF TAG END 15_cbbf13
					$content .="
					</div>
					";
				}
			}//IF TAG END 13_c4b784
			$count_4_e7e5bd++;
			//do all result vars replacement
			$content_4_e7e5bd.= CMS_polymod_definition_parsing::replaceVars($content, $replace);
		}
		$content = $content_4_e7e5bd; //retrieve previous content var if any
		$replace = $replace_4_e7e5bd; //retrieve previous replace vars if any
		$object[$objectDefinition_mediaresult] = $object_4_e7e5bd; //retrieve previous object search if any
	}
	//RESULT mediaresult TAG END 4_e7e5bd
	//destroy search and results mediaresult objects
	unset($search_mediaresult);
	unset($results_mediaresult);
	//SEARCH mediaresult TAG END 2_3b5962
