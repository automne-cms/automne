...
<h2>Principe de gestion des droits</h2>
	...
	<div class="imgRight">
		<?php //Generated by : $Id: 35.php,v 1.8 2010/02/01 16:08:05 sebastien Exp $
if(!APPLICATION_ENFORCES_ACCESS_CONTROL || (isset($cms_user) && is_a($cms_user, 'CMS_profile_user') && $cms_user->hasModuleClearance('pmedia', CLEARANCE_MODULE_VIEW))){
	...
	//SEARCH mediaresult TAG START 2_a6d882
	$objectDefinition_mediaresult = '2';
	if (!isset($objectDefinitions[$objectDefinition_mediaresult])) {
		$objectDefinitions[$objectDefinition_mediaresult] = new CMS_poly_object_definition($objectDefinition_mediaresult);
	}
	//public search ?
	$public_2_a6d882 = isset($public_search) ? $public_search : false;
	//get search params
	$search_mediaresult = new CMS_object_search($objectDefinitions[$objectDefinition_mediaresult], $public_2_a6d882);
	$launchSearch_mediaresult = true;
	//add search conditions if any
	if (isset($blockAttributes['search']['mediaresult']['item'])) {
		$values_3_9da2ea = array (
			'search' => 'mediaresult',
			'type' => 'item',
			'value' => 'block',
			'mandatory' => 'true',
		);
		$values_3_9da2ea['value'] = $blockAttributes['search']['mediaresult']['item'];
		if ($values_3_9da2ea['type'] == 'publication date after' || $values_3_9da2ea['type'] == 'publication date before') {
			//convert DB format to current language format
			$dt = new CMS_date();
			$dt->setFromDBValue($values_3_9da2ea['value']);
			$values_3_9da2ea['value'] = $dt->getLocalizedDate($cms_language->getDateFormat());
		}
		$launchSearch_mediaresult = (CMS_polymod_definition_parsing::addSearchCondition($search_mediaresult, $values_3_9da2ea)) ? $launchSearch_mediaresult : false;
	} elseif (true == true) {
		//search parameter is mandatory and no value found
		$launchSearch_mediaresult = false;
	}
	//RESULT mediaresult TAG START 4_e9ae90
	...
	if ($results_mediaresult) {
		$object_4_e9ae90 = (isset($object[$objectDefinition_mediaresult])) ? $object[$objectDefinition_mediaresult] : ""; //save previous object search if any
		$replace_4_e9ae90 = $replace; //save previous replace vars if any
		$count_4_e9ae90 = 0;
		$content_4_e9ae90 = $content; //save previous content var if any
		$maxPages_4_e9ae90 = $search_mediaresult->getMaxPages();
		$maxResults_4_e9ae90 = $search_mediaresult->getNumRows();
		foreach ($results_mediaresult as $object[$objectDefinition_mediaresult]) {
			$content = "";
			$replace["atm-search"] = array (
				"{resultid}" 	=> (isset($resultID_mediaresult)) ? $resultID_mediaresult : $object[$objectDefinition_mediaresult]->getID(),
				"{firstresult}" => (!$count_4_e9ae90) ? 1 : 0,
				"{lastresult}" 	=> ($count_4_e9ae90 == sizeof($results_mediaresult)-1) ? 1 : 0,
				"{resultcount}" => ($count_4_e9ae90+1),
				"{maxpages}"    => $maxPages_4_e9ae90,
				"{currentpage}" => ($search_mediaresult->getAttribute('page')+1),
				"{maxresults}"  => $maxResults_4_e9ae90,
			);
			//IF TAG START 5_c46d37
			...
			//IF TAG START 8_e1563a
			$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('fileExtension',''))." == 'flv'", $replace);
			if ($ifcondition) {
				$func = create_function("","return (".$ifcondition.");");
				if ($func()) {
					//IF TAG START 9_f20a8c
					$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
					if ($ifcondition) {
						$func = create_function("","return (".$ifcondition.");");
						if ($func()) {
			...
			if ($ifcondition) {
				...
				if ($func()) {
					...
					}//IF TAG END 12_ba0905
				}
			}//IF TAG END 11_0a1c3c
			//IF TAG START 13_dbcf09
			...
			if ($ifcondition) {
				...
				if ($func()) {
					...
					}//IF TAG END 14_f74936
					//IF TAG START 15_9d9e99
					$ifcondition = CMS_polymod_definition_parsing::replaceVars("!".CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
					if ($ifcondition) {
						$func = create_function("","return (".$ifcondition.");");
						if ($func()) {
					...
					}//IF TAG END 15_9d9e99
					$content .="
					</div>
					";
				}
			}//IF TAG END 13_dbcf09
			$count_4_e9ae90++;
			//do all result vars replacement
			$content_4_e9ae90.= CMS_polymod_definition_parsing::replaceVars($content, $replace);
		}
		$content = $content_4_e9ae90; //retrieve previous content var if any
		$replace = $replace_4_e9ae90; //retrieve previous replace vars if any
		$object[$objectDefinition_mediaresult] = $object_4_e9ae90; //retrieve previous object search if any
	}
	//RESULT mediaresult TAG END 4_e9ae90
	//destroy search and results mediaresult objects
	unset($search_mediaresult);
	unset($results_mediaresult);
	//SEARCH mediaresult TAG END 2_a6d882
