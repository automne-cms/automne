...
<h2>Principe de gestion des droits</h2>
	...
	<div class="imgRight">
		<?php //Generated by : $Id: 35.php,v 1.1 2009/03/06 11:01:49 sebastien Exp $
$content = "";
$replace = "";
...
//SEARCH mediaresult TAG START 2_21cf2e
$objectDefinition_mediaresult = '2';
if (!isset($objectDefinitions[$objectDefinition_mediaresult])) {
	$objectDefinitions[$objectDefinition_mediaresult] = new CMS_poly_object_definition($objectDefinition_mediaresult);
}
//public search ?
$public_2_21cf2e = isset($public_search) ? $public_search : false;
//get search params
$search_mediaresult = new CMS_object_search($objectDefinitions[$objectDefinition_mediaresult], $public_2_21cf2e);
$launchSearch_mediaresult = true;
//add search conditions if any
if ($blockAttributes['search']['mediaresult']['item']) {
	$values_3_8681e1 = array (
		'search' => 'mediaresult',
		'type' => 'item',
		'value' => 'block',
		'mandatory' => 'true',
	);
	$values_3_8681e1['value'] = $blockAttributes['search']['mediaresult']['item'];
	$launchSearch_mediaresult = (CMS_polymod_definition_parsing::addSearchCondition($search_mediaresult, $values_3_8681e1)) ? $launchSearch_mediaresult : false;
} elseif (true == true) {
	//search parameter is mandatory and no value found
	$launchSearch_mediaresult = false;
}
//RESULT mediaresult TAG START 4_15884d
...
if ($results_mediaresult) {
	$object_4_15884d = $object[$objectDefinition_mediaresult]; //save previous object search if any
	$replace_4_15884d = $replace; //save previous replace vars if any
	$count_4_15884d = 0;
	$content_4_15884d = $content; //save previous content var if any
	$maxPages_4_15884d = $search_mediaresult->getMaxPages();
	$maxResults_4_15884d = $search_mediaresult->getNumRows();
	foreach ($results_mediaresult as $object[$objectDefinition_mediaresult]) {
		$content = "";
		$replace["atm-search"] = array (
			"{resultid}" 	=> (isset($resultID_mediaresult)) ? $resultID_mediaresult : $object[$objectDefinition_mediaresult]->getID(),
			"{firstresult}" => (!$count_4_15884d) ? 1 : 0,
			"{lastresult}" 	=> ($count_4_15884d == sizeof($results_mediaresult)-1) ? 1 : 0,
			"{resultcount}" => ($count_4_15884d+1),
			"{maxpages}"    => $maxPages_4_15884d,
			"{currentpage}" => ($search_mediaresult->getAttribute('page')+1),
			"{maxresults}"  => $maxResults_4_15884d,
		);
		//IF TAG START 5_10693c
		...
		//IF TAG START 8_b23804
		$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('fileExtension',''))." == 'flv'", $replace);
		if ($ifcondition) {
			$func = create_function("","return (".$ifcondition.");");
			if ($func()) {
				//IF TAG START 9_ac58b3
				$ifcondition = CMS_polymod_definition_parsing::replaceVars(CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
				if ($ifcondition) {
					$func = create_function("","return (".$ifcondition.");");
					if ($func()) {
		...
		if ($ifcondition) {
			...
			if ($func()) {
				...
				}//IF TAG END 12_d404f5
			}
		}//IF TAG END 11_e27f7f
		//IF TAG START 13_94d753
		...
		if ($ifcondition) {
			...
			if ($func()) {
				...
				}//IF TAG END 14_978a10
				//IF TAG START 15_74e1ef
				$ifcondition = CMS_polymod_definition_parsing::replaceVars("!".CMS_polymod_definition_parsing::prepareVar($object[2]->objectValues(9)->getValue('thumbnail','')), $replace);
				if ($ifcondition) {
					$func = create_function("","return (".$ifcondition.");");
					if ($func()) {
		...
		}//IF TAG END 13_94d753
		$count_4_15884d++;
		//do all result vars replacement
		$content_4_15884d.= CMS_polymod_definition_parsing::replaceVars($content, $replace);
	}
	$content = $content_4_15884d; //retrieve previous content var if any
	$replace = $replace_4_15884d; //retrieve previous replace vars if any
	$object[$objectDefinition_mediaresult] = $object_4_15884d; //retrieve previous object search if any
}
//RESULT mediaresult TAG END 4_15884d
//destroy search and results mediaresult objects
unset($search_mediaresult);
unset($results_mediaresult);
//SEARCH mediaresult TAG END 2_21cf2e
