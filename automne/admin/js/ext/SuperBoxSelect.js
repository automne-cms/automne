...
Ext.ux.form.SuperBoxSelect = Ext.extend(Ext.ux.form.SuperBoxSelect,Ext.form.ComboBox,{
    ...
    addItemBox : function(itemVal,itemDisplay,itemCaption, itemClass, itemStyle) {
        ...
        hConfig = {
            tag :'input', 
            type :'hidden', 
            value : itemVal+(this.getValue() ? this.valueDelimiter + this.getValue() : ''),//itemVal,
		...
		//pr(this.el);
		var hidden = this.el.parent().child('input[type=hidden]');
		if (hidden) {
			hidden.dom.value = hConfig.value;
		} else {
			hidden = this.el.insertSibling(hConfig,'before');
		}
        box.hidden = hidden;
...
Ext.ux.form.SuperBoxSelectItem = Ext.extend(Ext.ux.form.SuperBoxSelectItem,Ext.Component, {
    ...
    preDestroy : function(supressEffect) {
    	...
    	var actionDestroy = function(){
            ...
            this.hidden.dom.value = this.owner.getValue();
