...
case "setParameters":
	...
	if (!$formok) {
		...
		if (!$cms_block->writeToPersistence($cms_page->getID(), $cs, $rowTag, RESOURCE_LOCATION_EDITION, false, array("value"=>$_POST['value']))) {
...
if (sizeof($blockParamsDefinition['search'])) {
	...
	<input type="hidden" name="module" value="'.$cms_module->getCodename().'" />';
	foreach ($blockParamsDefinition['search'] as $searchName => $searchParams) {
