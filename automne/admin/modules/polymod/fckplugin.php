...
// Add specific css and JS code if we use the resultsDefinition
if($resultsDefinition) {
	if (file_exists(PATH_CSS_FS.'/modules/'.$polyModuleCodename.'.css')){
		$content.='<link rel="stylesheet" type="text/css" href="'.PATH_CSS_WR.'/modules/'.$polyModuleCodename.'.css" />';
	}
	$jsFiles = $cms_module->getJSFiles();
	if ($jsFiles) {
		foreach ($jsFiles as $jsfile) {
			$content .= '<script type="text/javascript" src="'.$jsfile.'"></script>'."\n";
		}
	}
}
