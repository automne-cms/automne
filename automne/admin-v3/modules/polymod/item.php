...
// +----------------------------------------------------------------------+
//
// $Id: item.php,v 1.1.1.1 2008/11/26 17:12:06 sebastien Exp $
...
case "validate":
	...
	if (!$cms_message) {
		...
		if (!$cms_message) {
			header("Location: ".$back."?polymod=".$polyModuleCodename."&item=".$item->getId()."&object=".$object->getID()."&cms_message_id=".MESSAGE_ACTION_OPERATION_DONE."&".session_name()."=".session_id());
			exit;
		}
	}
	break;
...
if ($_POST["item"]) {
	$dialog->setBacklink($back."?polymod=".$polyModuleCodename."&item=".$item->getId()."&object=".$object->getID());
} else {
