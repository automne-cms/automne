...
class CMS_block_image extends CMS_block
{
	...
	*/
	public function _replaceBlockVars($data, $html_attributes, $location, $public) {
		...
		if ($data["enlargedFile"]) {
			$href = (OPEN_ZOOMIMAGE_IN_POPUP) ?
				CMS_websitesCatalog::getCurrentDomain(@$this->_pageID).PATH_REALROOT_WR . "/" . CMS_block_image::BLOCK_IMAGE_POPUP . '?location='.$folder.'&amp;file=' . $data["enlargedFile"] . '&amp;label=' . urlencode($data["label"]) :
				$domain.PATH_MODULES_FILES_STANDARD_WR.'/'.$folder.'/'.$data["enlargedFile"];
		...
		$replace = array(
			...
			'{{imagePath}}'		=> $domain.PATH_MODULES_FILES_STANDARD_WR.'/'.$folder,
			'{{imageName}}'		=> $data["file"],
