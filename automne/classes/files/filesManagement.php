...
class CMS_file extends CMS_grandFather
{
	...
	/**
		...
		* @access public
		*/
	public function setContent($content, $allowSetBlank=false) {
	...
	/**
		...
		* @access public
		*/
	public function getName($withPath = true)
	{
	...
	/**
		...
		* @access public
		*/
	public function getFilename($withPath = true)
	{
	...
	/**
		...
		* @access public
		*/
	public function getFileSize($unit = false)
	{
		if (!$this->exists()) {
			return false;
		}
	...
	/**
		...
		* @access public
		*/
	public function getFileIcon($from=self::FILE_SYSTEM) {
	...
	/**
		...
		* @access public
		*/
	public function getFilePath($from=self::FILE_SYSTEM) {
		$this->_basedir = dirname($this->_name);
	...
	/**
		...
		* @access public
		*/
	public function readContent($returnAs = "string", $array_map_function = "trim", $csvargs = array('delimiter' => ';', 'enclosure' => '"', 'strict' => true)) {
		if ($this->_exists) {
			if ($this->_type===self::TYPE_FILE) {
				if ($returnAs == "string") {
	...
	/**
		...
		* @access public
		*/
	public function writeToPersistence($allowWriteBlank = false, $createFolder = false)  {
		if ($this->_type==self::TYPE_FILE) {
	...
	/**
		...
		* @access public
		*/
	public function download($inline = true, $deleteFile = false, $forceContentType = false) {
		if (!$this->exists()) {
	 ...
	 * @param boolean $ignoreDirErrors, if set to true final status will ignore errors occuring while deleting directories // PMU: added in Automne v4.2.2
	 * @return boolean true on success, false on failure
	 * @static
	 */
	 ...
	 * @static
	 */
	public static function fileIsExecutable($f) {
		$f = realpath($f);
		if (!file_exists($f)) {
			return false;
		}
	 ...
	 * @static
	 */
	public static function chmodFile($right,$file)
	{
		$file = realpath($file);
		if (!file_exists($file)) {
			return false;
	...
	public static function getMaxUploadFileSize($unit = 'M') {
		...
		$configuredUnit = 'M';
		if(preg_match('/^(\d*)(M|G|K)/', $max,$matches)){
			$max = $matches[1];
			$configuredUnit = $matches[2];
	...
	/**
		...
		* @static
		*/
	public static function sendFiles($files, $contentType = 'text/html') {
	...
	/**
		...
		* @static
		*/
	public static function downloadFile($source, $inline = true, $deleteFile = false, $forceContentType = false) {
		if (!file_exists($source)) {
